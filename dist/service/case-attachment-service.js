...
const getAttachmentSnapshot = (record) => pickSnapshot(record, CASE_ATTACHMENT_FIELDS);
const __dirname = path.dirname(fileURLToPath(import.meta.url));
const CASE_UPLOAD_DIR = path.resolve(__dirname, "..", "..", "public", "assets", "case");
...
const normalizeMediaType = (value) => {
    ...
    if (!CASE_MEDIA_TYPES.includes(normalized)) {
...
const buildCaseFileName = (caseId, mediaType, timestamp, extension) => {
...
const ensureEmployeeCaseAccess = async (employeeId, caseId) => {
...
export class CaseAttachmentService {
    static async create(requesterId, reqBody) {
        ...
        if (hasFileData) {
            ...
            if (!filePath || !fileName) {
                throw new ResponseError(400, "filePath dan fileName wajib diisi jika fileData tidak disertakan");
            }
        }
        let created;
        try {
        ...
        await writeAuditLog({
            ...
            snapshot: getAttachmentSnapshot(created),
    ...
    static async update(requesterId, reqBody) {
        ...
        let fileMime = request.fileMime !== undefined
            ? normalizeNullableText(request.fileMime) ?? null
            : existing.fileMime;
        let fileSize = request.fileSize !== undefined ? request.fileSize : existing.fileSize;
        let newFileFullPath = null;
        let oldFileFullPath = null;
        let oldFileRenamedPath = null;
        if (request.fileData) {
            const { base64, mime } = parseFileData(request.fileData);
            const fileBuffer = Buffer.from(base64, "base64");
            if (!fileBuffer.length) {
        ...
        const nextCaption = request.caption !== undefined
            ? normalizeNullableText(request.caption) ?? null
            : existing.caption;
        const nextLocationDesc = request.locationDesc !== undefined
        ...
        const changes = buildChanges(before, updated, CASE_ATTACHMENT_FIELDS);
        if (changes.length > 0) {
            await writeAuditLog({
                module: "CASE",
                entity: "CASE_ATTACHMENT",
    ...
    static async softDelete(requesterId, reqBody) {
        ...
        await writeAuditLog({
            ...
            actorId: requesterId,
            actorType: resolveActorType(requesterId),
            snapshot: getAttachmentSnapshot(updated),
            meta: { caseId: updated.caseId },
        });
    ...
    static async getFile(requesterId, caseAttachmentId) {
        const access = await resolveCaseAccess(requesterId);
        assertCaseRead(access);
...
//# sourceMappingURL=case-attachment-service.js.map