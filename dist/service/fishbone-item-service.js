...
import { generateFishboneItemId, generateFishboneItemCauseId, } from "../utils/id-generator.js";
...
import { toFishboneItemResponse, toFishboneItemListResponse, } from "../model/fishbone-item-model.js";
...
const ensureCausesExist = async (fishboneId, causeIds) => {
...
const syncItemCauseLinks = async (fishboneItemId, causeIds, requesterId) => {
    const auditPayload = {
        created: [],
        reactivated: [],
        deleted: [],
...
export class FishboneItemService {
    static async create(requesterId, reqBody) {
        ...
        const normalizedCauseIds = await ensureCausesExist(request.fishboneId, request.causeIds);
    ...
    static async update(requesterId, reqBody) {
        ...
        if (request.causeIds !== undefined) {
            normalizedCauseIds = await ensureCausesExist(existing.fishboneId, request.causeIds);
        }
        ...
        if (normalizedCauseIds) {
            const auditPayload = await syncItemCauseLinks(updated.fishboneItemId, normalizedCauseIds, requesterId);
...
//# sourceMappingURL=fishbone-item-service.js.map