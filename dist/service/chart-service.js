...
export class ChartService {
    ...
    static async update(requesterUserId, request) {
        ...
        if (!existing)
            throw new ResponseError(404, "Chart ID not found");
        ...
        if (diff < 0) {
            ...
            if (filledSlots.length > finalCapacity) {
                ...
                const filledInfo = filledSlots.map(slot => {
                    const emp = employees.find(e => e.UserId === slot.userId);
                    return {
                ...
                throw new ResponseError(400, `Cannot reduce capacity to ${finalCapacity}. ` +
    ...
    static async softDelete(requesterUserId, request) {
        ...
        if (child) {
            throw new ResponseError(400, "Cannot delete: this chart still has children. Move or delete them first.");
        }
        ...
        if (occupiedMember) {
            ...
            throw new ResponseError(400, `Cannot delete chart: there is at least one assigned member (${name}). ` +
    ...
    // 📋 LIST BY SBU / SUB
    // ================================
    static async listBySbuSub(requesterId, pilarId, sbuId, sbuSubId) {
        ...
        if (pilarId !== undefined)
            whereClause.pilarId = pilarId;
        if (sbuId !== undefined)
            whereClause.sbuId = sbuId;
        const nodes = await prismaFlowly.chart.findMany({
            where: whereClause,
            orderBy: { orderIndex: "asc" },
        });
