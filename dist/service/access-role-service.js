...
const ensureSubjectExists = async (subjectType, subjectId) => {
    if (subjectType === "ROLE") {
...
const ensureMasterAccessRole = async (resourceType, masAccessId, resourceKey) => {
...
const parseOrgResourceId = (resourceType, resourceKey) => {
...
const ensureOrgResourceExists = async (resourceType, resourceKey) => {
    if (!orgResourceTypes.has(resourceType)) {
        return;
    }
    ...
    if (resourceType === "PILAR") {
        ...
        if (!pilar)
            throw new ResponseError(400, "Pilar not found");
        return;
    }
    if (resourceType === "SBU") {
        ...
        if (!sbu)
            throw new ResponseError(400, "SBU not found");
        return;
    }
    if (resourceType === "SBU_SUB") {
        ...
        if (!sbuSub)
            throw new ResponseError(400, "SBU SUB not found");
    }
};
const resolveEmployeeIdForFocus = async (requesterId, requester) => {
...
const resolveFocusPilarIds = async (requesterId, requester) => {
...
export class AccessRoleService {
    ...
    static async update(requesterId, reqBody) {
        ...
        const masterInfo = await ensureMasterAccessRole(finalResourceType, finalMasAccessId ?? null, finalResourceKey ?? null);
        if (masterInfo.resourceKey) {
    ...
    static async getSummary(requesterId) {
        const requester = await prismaFlowly.user.findUnique({
            where: { userId: requesterId },
            include: { role: true }
        });
        ...
        const roleAccess = accessRoles.filter((access) => normalizeRequiredUpper(access.subjectType) === "ROLE");
        const userAccess = accessRoles.filter((access) => normalizeRequiredUpper(access.subjectType) === "USER");
...
//# sourceMappingURL=access-role-service.js.map