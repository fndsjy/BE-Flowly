...
const shouldReplaceMachine = (current, candidate) => {
    ...
    const candidateScore = (candidate.enabled ? 2 : 0) + (candidate.machineAlias ? 1 : 0);
    if (candidateScore !== currentScore) {
        return candidateScore > currentScore;
    }
    return candidate.id < current.id;
};
const dedupeFingerMachines = (machines) => {
    const byIp = new Map();
    ...
    return [...byIp.values()].sort((left, right) => {
        ...
        const aliasCompare = (left.machineAlias ?? "").localeCompare(right.machineAlias ?? "", "id", {
            sensitivity: "base",
        });
        if (aliasCompare !== 0) {
            return aliasCompare;
...
//# sourceMappingURL=finger-machine-service.js.map