...
const getMaterialFileTypeOrder = (fileType) => {
    if (fileType === 3) {
        return 0;
    }
    if (fileType === 2) {
...
const filterSelectedFiles = (files, metadata) => {
...
const serializeSelectionNote = (sourceMaterial, selectedFileIds) => {
    ...
    const normalizedSelectedIds = Array.from(new Set(selectedFileIds.filter((value) => Number.isInteger(value) && value > 0 && allowedIds.has(value)))).sort((left, right) => left - right);
...
export class OnboardingMaterialService {
    static async listSourceMaterials() {
        const materialRows = await prismaEmployee.$queryRaw `
      SELECT
        m.id AS materialId,
      ...
      ORDER BY
        ...
        mf.id ASC
    `;
        const materialTypeRows = await prismaEmployee.$queryRaw `
      SELECT
    ...
    `;
        ...
        for (const row of materialTypeRows) {
            ...
            const existing = typeMap.get(row.materialId) ?? new Set();
        ...
        for (const row of materialRows) {
            ...
            if (!material) {
                const seededTypeNames = new Set(typeMap.get(row.materialId) ?? []);
                material = {
        ...
        return Array.from(materialMap.values()).map((material) => {
            ...
            const materialTypes = Array.from(material.typeNames).sort((left, right) => left.localeCompare(right));
            return {
        ...
        for (const portalTemplate of portalTemplates) {
            for (const stageTemplate of portalTemplate.stageTemplates) {
                for (const stageMaterial of stageTemplate.stageMaterials) {
                    const existingPortals = sharedPortalMap.get(stageMaterial.materiId) ?? new Set();
            ...
            for (const stageTemplate of portalTemplate.stageTemplates) {
                for (const stageMaterial of stageTemplate.stageMaterials) {
                    ...
                    assignments.push({
                        ...
                        selectedFileIds: selectionMetadata.mode === "SELECTED"
                            ? selectionMetadata.selectedFileIds
                            : sourceFiles.map((file) => file.id),
                        fileSelectionMode: selectionMetadata.mode === "SELECTED" && selectedFiles.length < sourceFiles.length
                            ? "SELECTED"
                            : "ALL",
                        files: selectedFiles,
                        portalKey: portalTemplate.portalKey,
    ...
    static async createAssignment(requesterId, reqBody) {
        const request = Validation.validate(OnboardingMaterialValidation.CREATE_ASSIGNMENT, reqBody);
        await ensureAdminAccess(requesterId);
        ...
        const selectionNote = serializeSelectionNote(sourceMaterial, request.selectedFileIds);
        const now = new Date();
        const nextOrderIndex = (stageTemplate.stageMaterials[0]?.orderIndex ?? 0) + 10;
        if (existing) {
    ...
    static async deleteAssignment(requesterId, reqBody) {
        const request = Validation.validate(OnboardingMaterialValidation.DELETE_ASSIGNMENT, reqBody);
        await ensureAdminAccess(requesterId);
...
//# sourceMappingURL=onboarding-material-service.js.map