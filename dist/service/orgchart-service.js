...
export class OrgChartService {
    ...
    static async create(requesterUserId, request) {
        ...
        if (parentId) {
            ...
            if (parentNode.structureId !== structureId) {
                throw new ResponseError(400, `Parent ${parentId} in ${parentNode.structureId}, not in ${structureId}`);
            }
        }
        // Validate parentId
    ...
    static async update(requesterUserId, request) {
        ...
        * -------------------------------------------- */
        ...
        if (structureId && structureId !== existing.structureId) {
            ...
            if (childExists) {
                throw new ResponseError(400, "Cannot change structure because this node still has children.");
            }
        }
        /* --------------------------------------------
        * 5. VALIDATE USER ID
    ...
    static async softDelete(requesterUserId, request) {
        ...
        * -------------------------------------------- */
        ...
        if (child) {
            throw new ResponseError(400, "Cannot delete node: this position still has subordinates (children). Move or delete all children first.");
        }
        /* --------------------------------------------
        * 4. OPTIONAL: CEK ROOT (parentId null)
        * -------------------------------------------- */
        if (node.parentId === null) {
            throw new ResponseError(400, "Cannot delete root node. Assign a new root or move all nodes first.");
        }
        /* --------------------------------------------
        * 5. SOFT DELETE
        * -------------------------------------------- */
