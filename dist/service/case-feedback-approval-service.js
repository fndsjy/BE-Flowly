...
export class CaseFeedbackApprovalService {
    static async approve(requesterId, reqBody) {
        const request = Validation.validate(CaseFeedbackApprovalValidation.APPROVE, reqBody);
        const access = await resolveCaseAccess(requesterId);
        if (access.actorType !== "EMPLOYEE" || access.employeeId === undefined) {
            throw new ResponseError(403, "Only PIC of requester SBU Sub can approve");
        }
...
//# sourceMappingURL=case-feedback-approval-service.js.map