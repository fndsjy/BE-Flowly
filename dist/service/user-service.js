...
import { invalidateProfileCache, withProfileCache, } from "../application/profile-cache.js";
...
const isInactiveEmployeeStatus = (value) => {
...
const profileSlowThresholdMs = parsePositiveInteger(process.env.PROFILE_SLOW_THRESHOLD_MS, 2000);
const profilePerfNow = () => process.hrtime.bigint();
const profilePerfElapsedMs = (startedAt) => Math.round((Number(process.hrtime.bigint() - startedAt) / 1_000_000) * 100) /
    100;
const shouldLogProfilePerf = (totalMs) => profilePerfVerbose || totalMs >= profileSlowThresholdMs;
const logProfilePerf = (userId, totalMs, trace) => {
    if (!shouldLogProfilePerf(totalMs)) {
        return;
    }
    const level = totalMs >= profileSlowThresholdMs ? logger.warn.bind(logger) : logger.info.bind(logger);
    level("[PROFILE PERF]", {
        userId,
        totalMs,
...
const findDepartmentName = async (departmentId, trace) => {
...
const findFlowlyUserById = async (userId, trace) => {
    const startedAt = profilePerfNow();
...
const toProfileResponse = async (context, trace) => {
...
export class UserService {
    ...
    static async login(request) {
        ...
        const identity = loginRequest.identity?.trim() ||
        ...
        if (isInactiveEmployeeStatus(employee.status)) {
            throw new ResponseError(403, "Akun karyawan sudah nonaktif. Silakan hubungi HRD.");
        }
