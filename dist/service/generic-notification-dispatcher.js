...
const getMaxAttempts = () => {
    const value = Number(process.env.NOTIFICATION_MAX_ATTEMPTS ??
...
const markOutboxSent = async (id, attempts, provider) => {
    await prismaFlowly.notificationOutbox.update({
        where: { notificationOutboxId: id },
        data: {
            attempts,
...
const dispatchOutboxItem = async (item) => {
    ...
    if (!message) {
        await markOutboxFailed(item.notificationOutboxId, attempts, "Missing notification message");
        return;
    }
    if (channel === CHANNEL_WHATSAPP) {
        if (!item.phoneNumber.trim()) {
            await markOutboxFailed(item.notificationOutboxId, attempts, "Missing phone number", { provider: CHANNEL_WHATSAPP });
            return;
        }
    ...
    await markOutboxFailed(item.notificationOutboxId, attempts, `Unsupported notification channel: ${channel || "UNKNOWN"}`, {
    ...
    try {
        ...
        for (const item of items) {
            try {
                ...
                await markOutboxFailed(item.notificationOutboxId, item.attempts + 1, buildErrorMessage(error));
            }
        }
    }
    catch (error) {
...
//# sourceMappingURL=generic-notification-dispatcher.js.map