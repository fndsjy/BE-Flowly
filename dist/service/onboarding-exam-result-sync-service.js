...
const uniqueTexts = (values) => Array.from(new Set((values ?? [])
    .map((value) => value.trim())
    .filter((value) => value.length > 0)));
const isAssignmentBlockedFromSync = (status) => {
...
const isStagePassedForSync = (status) => {
    const normalized = normalizeUpper(status);
    return normalized === "PASSED" || normalized === "COMPLETED";
};
const resolveOpenAssignmentStatus = (stageStatus) => {
...
const fallbackPassedNotificationTemplates = {
...
const normalizePassedNotificationMessage = (recipientRole, messageTemplate) => {
...
const toNotificationTemplates = (templates, recipientRole) => {
    const selected = templates.map((template) => ({
        notificationTemplateId: template.notificationTemplateId,
        channel: template.channel,
        messageTemplate: normalizePassedNotificationMessage(recipientRole, template.messageTemplate),
    }));
...
const enqueueOnboardingPassedNotifications = async (params) => {
    try {
        ...
        const portalName = normalizeNote(assignment.portalTemplate?.portalName) ?? portalKey;
        const decisionNote = params.note ?? "Lulus otomatis setelah menyelesaikan semua tahap onboarding.";
        const [participantRuntimeTemplates, hrdRuntimeTemplates, picRuntimeTemplates] = await Promise.all([
        ...
        const participantTemplates = toNotificationTemplates(participantRuntimeTemplates, PARTICIPANT_RECIPIENT_ROLE);
        const hrdTemplates = toNotificationTemplates(hrdRuntimeTemplates, HRD_RECIPIENT_ROLE);
        const picTemplates = toNotificationTemplates(picRuntimeTemplates, SBU_SUB_PIC_RECIPIENT_ROLE);
        ...
        const existingRecipientKeys = new Set(existingOutboxes.map((item) => `${normalizeUpper(item.recipientRole)}|${item.recipientReferenceId ?? ""}`));
        ...
        if (participantTemplates.length > 0 &&
            ...
            for (const template of participantTemplates) {
                ...
                const phoneNumber = channel === CHANNEL_WHATSAPP ? participantPhoneNumber ?? "" : "";
                const email = channel === CHANNEL_EMAIL ? participantEmail ?? "" : participantEmail;
        ...
        if (hrdTemplates.length > 0) {
            for (const recipient of hrdRecipients) {
                if (existingRecipientKeys.has(`${HRD_RECIPIENT_ROLE}|${recipient.userId}`)) {
                    continue;
                }
                const context = {
                    ...baseContext,
        ...
        if (picTemplates.length > 0) {
            for (const recipient of picRecipients) {
                if (existingRecipientKeys.has(`${SBU_SUB_PIC_RECIPIENT_ROLE}|${recipient.recipientUserId}`)) {
                    continue;
                }
...
const getAssignmentIdsForSyncScope = async (options) => {
    ...
    const shouldQueryScope = options.participantReferenceIds.length > 0 || options.portalKeys.length > 0;
    if (shouldQueryScope) {
...
const reconcileAssignmentProgress = async (onboardingAssignmentId) => {
    const assignment = await prismaFlowly.onboardingAssignment.findUnique({
        where: {
    ...
    const allStagesPassed = activeStages.every((stage) => isStagePassedForSync(stage.status));
    ...
    const firstIncompleteStage = activeStages.find((stage) => !isStagePassedForSync(stage.status));
    ...
    const shouldUpdate = currentStatus !== nextStatus ||
...
export class OnboardingExamResultSyncService {
    static async syncReleasedResults(options = {}) {
        const participantReferenceIds = uniqueTexts(options.participantReferenceIds);
        const portalKeys = uniqueTexts(options.portalKeys).map((value) => value.toUpperCase());
        const examSessionIds = uniqueTexts(options.examSessionIds);
        const where = {
            isDeleted: false,
            employeeExamSessionId: examSessionIds.length > 0 ? { in: examSessionIds } : { not: null },
            status: {
        ...
        const stageProgressIds = uniqueTexts(attempts.map((attempt) => attempt.onboardingStageProgressId));
        const allAttemptsForStageProgress = stageProgressIds.length > 0
            ...
            : [];
        const sessionIds = uniqueTexts(attempts.map((attempt) => attempt.employeeExamSessionId ?? ""));
        const releasedSessions = sessionIds.length > 0
            ...
            : [];
        const sessionMap = new Map(releasedSessions
            .filter((session) => session.exams_id)
            .map((session) => [session.exams_id ?? "", session]));
        const latestAttemptNoByStageProgress = new Map();
        const attemptsByStageProgress = new Map();
        for (const attempt of allAttemptsForStageProgress) {
            const stageAttempts = attemptsByStageProgress.get(attempt.onboardingStageProgressId) ?? [];
            stageAttempts.push(attempt);
            attemptsByStageProgress.set(attempt.onboardingStageProgressId, stageAttempts);
            const latestAttemptNo = latestAttemptNoByStageProgress.get(attempt.onboardingStageProgressId) ?? 0;
            if (attempt.attemptNo > latestAttemptNo) {
                latestAttemptNoByStageProgress.set(attempt.onboardingStageProgressId, attempt.attemptNo);
            }
        }
        let synced = 0;
        ...
        for (const attempt of attempts) {
            ...
            const passScore = Math.max(0, Number(attempt.stageExam.passScore ?? DEFAULT_PASS_SCORE));
            const isPassed = score >= passScore;
            const nextStatus = isPassed ? "PASSED" : "REMEDIAL";
            const nextRemedialCount = (attemptsByStageProgress.get(attempt.onboardingStageProgressId) ?? []).filter((stageAttempt) => {
                const status = stageAttempt.onboardingExamAttemptId === attempt.onboardingExamAttemptId
                    ? nextStatus
                    : stageAttempt.status;
...
//# sourceMappingURL=onboarding-exam-result-sync-service.js.map