...
import { toProcedureSopIkResponse, toProcedureSopIkListResponse, } from "../model/procedure-sop-ik-model.js";
...
const normalizeIkIds = (ikIds) => Array.from(new Set(ikIds.map((id) => id.trim()).filter((id) => id.length > 0)));
const fetchLinks = async (filters) => prismaFlowly.procedureSopIK.findMany({
    where: filters,
    include: {
        masterIK: {
            select: {
...
export class ProcedureSopIkService {
    static async create(requesterId, reqBody) {
        ...
        for (const link of toReactivate) {
            ...
            const changes = buildChanges(link, after, SOP_IK_AUDIT_FIELDS);
            if (changes.length > 0) {
                await writeAuditLog({
                    module: "PROCEDURE",
                    entity: "SOP_IK",
    ...
    static async update(requesterId, reqBody) {
        ...
        const changes = buildChanges(existing, updated, SOP_IK_AUDIT_FIELDS);
        if (changes.length > 0) {
            await writeAuditLog({
                module: "PROCEDURE",
                entity: "SOP_IK",
    ...
    static async softDelete(requesterId, reqBody) {
        ...
        await writeAuditLog({
            ...
            actorId: requesterId,
            actorType: access.actorType,
            snapshot: getSopIkSnapshot(existing),
...
//# sourceMappingURL=procedure-sop-ik-service.js.map