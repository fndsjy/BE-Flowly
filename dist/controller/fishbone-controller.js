...
export class FishboneController {
    static async create(req, res, next) {
        try {
            ...
            const response = await FishboneService.create(payload.userId, req.body);
    ...
    static async update(req, res, next) {
        try {
            ...
            if (!token)
                throw new ResponseError(401, "Unauthorized");
            const payload = verifyToken(token);
            const response = await FishboneService.update(payload.userId, req.body);
    ...
    static async softDelete(req, res, next) {
        try {
            ...
            if (!token)
                throw new ResponseError(401, "Unauthorized");
            const payload = verifyToken(token);
            const response = await FishboneService.softDelete(payload.userId, req.body);
            res.status(200).json({ response });
        }
        catch (err) {
            next(err);
    ...
    static async list(req, res, next) {
        try {
            ...
            const response = await FishboneService.list(payload.userId, Object.keys(filters).length > 0 ? filters : undefined);
            res.status(200).json({ response });
        }
        catch (err) {
            next(err);
...
//# sourceMappingURL=fishbone-controller.js.map