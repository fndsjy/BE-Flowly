...
const DEFAULT_TYPE_ORDER: AdminOnboardingExamQuestionCategory[] = [
  "MCQ",
  "ESSAY",
  "TRUE_FALSE",
  "POLLING",
...
): AdminOnboardingExamQuestionCategory | null => {
...
const isTrueFalseQuestion = (options: string[]) => {
  ...
  const normalized = options
    .map((option) =>
      normalizeAnswerValue(option)?.replace(/^[A-Z]\s*[\.\)\-:]\s*/i, "") ?? ""
    )
  ...
  return (
    (optionSet.has("true") && optionSet.has("false")) ||
    (optionSet.has("benar") && optionSet.has("salah"))
  );
};

const resolveQuestionCategory = (options: string[]) => {
  if (options.length === 0) {
    return "ESSAY";
  }
...
const normalizeTypeOrder = (
  ...
  const fromInput = Array.isArray(value)
    ? value.filter(
        (item): item is AdminOnboardingExamQuestionCategory =>
          typeof item === "string" &&
          allowed.has(item as AdminOnboardingExamQuestionCategory)
      )
    : [];
...
const filterSelectedQuestions = (
  questions: OnboardingExamQuestionResponse[],
  metadata: QuestionSelectionMetadata
) => {
...
const buildSourceExamMap = async () => {
  ...
  if (exams.length === 0) {
    return new Map<number, OnboardingSourceExamResponse>();
  }

  const questionTypeMap = new Map<number, AdminOnboardingExamQuestionCategory>();
  ...
  const questionMap = new Map<number, OnboardingExamQuestionResponse[]>();
  for (const question of questions) {
    ...
    const matchedOption =
      formattedOptions.find((option) =>
        correctAnswerLookup.some(
          (lookup) =>
            lookup === option.label ||
            lookup === option.choice.toLowerCase()
        )
      ) ?? null;

    const options = formattedOptions.map((option) => option.display);
    const category =
      (question.question_type != null ? questionTypeMap.get(question.question_type) : null) ??
      resolveQuestionCategory(options);

    const nextQuestion: OnboardingExamQuestionResponse = {
  ...
  const sourceExamMap = new Map<number, OnboardingSourceExamResponse>();
  for (const exam of exams) {
    const sourceQuestions = [...(questionMap.get(exam.id) ?? [])].sort(
    ...
    sourceExamMap.set(exam.id, {
      ...
      questionTypes,
      totalScore: sourceQuestions.reduce(
...
export class OnboardingExamService {
  ...
  static async listAssignments(
    ...
    const portals = [...portalTemplates]
      ...
      .map<OnboardingExamPortalResponse>((portalTemplate) => ({
    ...
    const assignments: OnboardingExamAssignmentResponse[] = [];
    for (const portalTemplate of portalTemplates) {
      ...
      for (const stageTemplate of portalTemplate.stageTemplates) {
        for (const stageExam of stageTemplate.stageExams) {
          ...
          const selectedQuestions = filterSelectedQuestions(
            sourceQuestions,
            selectionMetadata
          );
          ...
          assignments.push({
            ...
            questionTypes: Array.from(
              ...
              (left, right) =>
                (TYPE_ORDER_WEIGHT.get(left) ?? 999) -
  ...
  static async createAssignment(
    requesterId: string,
    reqBody: CreateOnboardingStageExamRequest
  ) {
    const request = Validation.validate(
      OnboardingExamValidation.CREATE_ASSIGNMENT,
      reqBody
    ) as CreateOnboardingStageExamRequest;
  ...
  static async updateStagePassScore(
    requesterId: string,
    reqBody: UpdateOnboardingStagePassScoreRequest
  ) {
    const request = Validation.validate(
      OnboardingExamValidation.UPDATE_STAGE_PASS_SCORE,
      reqBody
    ) as UpdateOnboardingStagePassScoreRequest;
  ...
  static async updateStageTypeOrder(
    requesterId: string,
    reqBody: UpdateOnboardingStageTypeOrderRequest
  ) {
    const request = Validation.validate(
      OnboardingExamValidation.UPDATE_STAGE_TYPE_ORDER,
      reqBody
    ) as UpdateOnboardingStageTypeOrderRequest;
  ...
  static async deleteAssignment(
    requesterId: string,
    reqBody: DeleteOnboardingStageExamRequest
  ) {
    const request = Validation.validate(
      OnboardingExamValidation.DELETE_ASSIGNMENT,
      reqBody
    ) as DeleteOnboardingStageExamRequest;
