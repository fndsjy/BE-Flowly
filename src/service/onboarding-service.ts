...
import {
  ...
  type EmployeeOnboardingPlacementDetailResponse,
  type EmployeeOnboardingStartItem,
  type EmployeeOnboardingSummaryResponse,
  type ListEmployeeOnboardingSummaryRequest,
  type MyOnboardingWorkspaceResponse,
  type OnboardingWorkspaceCertificateResponse,
  type StartOnboardingMaterialReadRequest,
  type StartOnboardingMaterialReadResponse,
  type OnboardingWorkspaceMaterialFileResponse,
  type StartEmployeeOnboardingRequest,
  type StartEmployeeOnboardingResponse,
} from "../model/onboarding-model.js";
...
import {
  ...
  generateOnboardingAssignmentId,
  generateOnboardingDecisionId,
  generateOnboardingMaterialProgressId,
  generateOnboardingStageProgressId,
...
import {
  ensureHrdCrudAccess,
  ensureHrdReadAccess,
  resolveHrdAccessLevel,
} from "../utils/hrd-access.js";
...
const DEFAULT_CERTIFICATE_ASSET_BASE_URL =
  "https://lms.domas.co.id/assets/img/sertifikat/peserta";
...
const ONBOARDING_PLACEMENT_REQUIRED_REASON =
  "Karyawan belum terdaftar di struktur organisasi atau sebagai PIC pilar/SBU/SBU SUB";
...
type OnboardingPlacementSource =
  (typeof ONBOARDING_PLACEMENT_SOURCES)[keyof typeof ONBOARDING_PLACEMENT_SOURCES];

type OnboardingPlacementInfo = {
  hasOnboardingPlacement: boolean;
  sources: OnboardingPlacementSource[];
  details: EmployeeOnboardingPlacementDetailResponse[];
};

type MutableOnboardingPlacementInfo = {
  sources: Set<OnboardingPlacementSource>;
  details: EmployeeOnboardingPlacementDetailResponse[];
  detailKeys: Set<string>;
};

type AdminMonitoringEmployeeSnapshot = {
  UserId: number;
  CardNo: string | null;
...
type WorkspaceMaterialSelectionMetadata = {
  mode: "ALL" | "SELECTED";
  selectedFileIds: number[];
  rawNote: string | null;
};

type WorkspaceExamSelectionMetadata = {
  mode: "ALL" | "SELECTED";
  selectedQuestionIds: number[];
};

type PicOnboardingScope = {
  employeeId: number;
  pilarIds: Set<number>;
  sbuIds: Set<number>;
  sbuSubIds: Set<number>;
};
...
const normalizeExamScore = (value: unknown) => {
...
const resolveCertificateAssetBaseUrl = () =>
  (normalizeNote(process.env.LMS_CERTIFICATE_ASSET_BASE_URL) ??
    DEFAULT_CERTIFICATE_ASSET_BASE_URL).replace(/\/+$/, "");

const encodeCertificatePath = (fileName: string) =>
  fileName
    .split("/")
    .map((part) => encodeURIComponent(part))
    .join("/");

const toCertificatePdfFileName = (fileName: string) => {
...
const buildCertificateAssetUrls = (fileName: string) => {
...
const buildEmployeeFirstLoginPassword = (
...
const resolveTemporaryPassword = (value?: string | null) => {
  const trimmed = value?.trim();
  if (!trimmed) {
    return "";
  }
...
const trimAuditActor = (value: string) => {
  const trimmed = value.trim();
  return trimmed.length > 0 ? trimmed.slice(0, 20) : null;
};

const addDays = (value: Date, day: number) =>
  new Date(value.getTime() + day * 24 * 60 * 60 * 1000);

const hasMeaningfulDate = (value?: Date | string | null) => {
  if (!value) {
    return false;
  }
...
const isFinalAssignmentStatus = (status?: string | null) => {
  const normalized = status?.trim().toUpperCase();
  return normalized ? FINAL_ASSIGNMENT_STATUSES.has(normalized) : false;
};

const isCompletedAssignmentStatus = (status?: string | null) => {
  const normalized = status?.trim().toUpperCase();
  return normalized
...
const ensureAdminMonitoringAccess = async (requesterUserId: string) => {
...
const resolveRequesterEmployeeId = async (requesterUserId: string) => {
...
const resolvePicOnboardingScope = async (
  requesterUserId: string
): Promise<PicOnboardingScope | null> => {
  ...
  const pilarIds = new Set<number>();
  const sbuIds = new Set<number>();
  const sbuSubIds = new Set<number>();
...
const buildPicParticipantEmployeeIds = async (scope: PicOnboardingScope) => {
  const employeeIds = new Set<number>();
  const addEmployeeId = (value?: number | null) => {
    if (Number.isInteger(value) && Number(value) > 0 && value !== scope.employeeId) {
      employeeIds.add(Number(value));
    }
  };

  const chartWhere: Prisma.ChartWhereInput[] = [];
...
const ensurePicCanAccessParticipant = async (
  requesterUserId: string,
  participantReferenceType: string,
  participantReferenceId: string
) => {
...
const ensureDirectSbuSubPicCanAccessParticipant = async (
  requesterUserId: string,
  participantReferenceType: string,
  participantReferenceId: string
) => {
  ...
  const employeeIds = await buildDirectSbuSubPicParticipantEmployeeIds(
    requesterUserId
  );
  return employeeIds.has(participantEmployeeId);
};
...
const getAdminPortalOrder = (portalKey: string) => {
  const index = ONBOARDING_ADMIN_PORTAL_KEYS.findIndex(
    (item) => item === normalizePortalKey(portalKey)
  );
  return index >= 0 ? index : Number.MAX_SAFE_INTEGER;
};

const renderTemplate = (template: string, context: Record<string, unknown>) =>
  template.replace(/\{badgeNumber\}/g, String(context.cardNumber ?? "")).replace(/\{(\w+)\}/g, (_, key: string) => {
...
const parseWorkspaceSelectionMetadata = (
  note: string | null | undefined
): WorkspaceMaterialSelectionMetadata => {
...
const parseWorkspaceExamSelectionMetadata = (
  note: string | null | undefined
): WorkspaceExamSelectionMetadata => {
  const normalizedNote = normalizeNote(note);
  if (!normalizedNote) {
    return {
      mode: "ALL",
...
const filterWorkspaceSelectedFiles = (
  files: OnboardingWorkspaceMaterialFileResponse[],
  metadata: WorkspaceMaterialSelectionMetadata
) => {
...
type WorkspaceMaterialProgressSnapshot = {
...
type WorkspaceExamQuestionSnapshot = {
  id: number;
  exam_id: number | null;
...
type WorkspaceStageExamSnapshot = {
...
const buildWorkspaceExamSummaries = (
  stageExams: WorkspaceStageExamSnapshot[],
  examNameMap: Map<number, string>,
  questionsByExam: Map<number, WorkspaceExamQuestionSnapshot[]>,
  questionTypeMap: Map<number, string>,
  stageDurationSeconds?: number
) =>
  stageExams.map((stageExam, index) => {
    ...
    const fallbackDurationSeconds = questionsToUse.reduce(
      (sum, question) =>
    ...
    return {
      ...
      durationSeconds:
        stageDurationSeconds && stageDurationSeconds > 0
          ? index === 0
...
) => `${onboardingStageMaterialId}:${Number(sourceFileId ?? 0)}`;

type WorkspaceSourceMaterial = Awaited<
  ReturnType<typeof OnboardingMaterialService.listSourceMaterials>
>[number];

const resolveSelectedSourceFileIds = (
  ...
  const selectedFileIds = new Set(metadata.selectedFileIds);
  const filtered = sourceFileIds.filter((sourceFileId) =>
    selectedFileIds.has(sourceFileId)
  );
  return filtered.length > 0 ? filtered : sourceFileIds;
};

const hasMaterialReadSignal = (
  progress:
    | Pick<
        WorkspaceMaterialProgressSnapshot,
        "status" | "readAt" | "lastReadAt" | "completedAt" | "openCount"
      >
    | null
    | undefined
) =>
...
const areStageMaterialsReadyForExam = (params: {
  stageMaterials: Array<{
    onboardingStageMaterialId: string;
    materiId: number;
    note: string | null;
  }>;
  materialProgresses: WorkspaceMaterialProgressSnapshot[];
  sourceMaterialMap: Map<number, WorkspaceSourceMaterial>;
}) => {
  const progressMap = new Map(
    params.materialProgresses.map((progress) => [
      buildMaterialProgressKey(
        progress.onboardingStageMaterialId,
        progress.sourceFileId
      ),
      progress,
    ])
  );
  ...
  return params.stageMaterials.every((stageMaterial) => {
    const sourceFileIds =
      params.sourceMaterialMap
        .get(stageMaterial.materiId)
        ?.files.map((file) => file.id) ?? [];
    const selectedFileIds = resolveSelectedSourceFileIds(
      sourceFileIds,
      stageMaterial.note
    );
    ...
    return selectedFileIds.every((sourceFileId) =>
      hasMaterialReadSignal(
        progressMap.get(
          buildMaterialProgressKey(
            stageMaterial.onboardingStageMaterialId,
            sourceFileId
          )
        )
      )
...
const getMostRecentDate = (values: Array<Date | null | undefined>) =>
...
const getEarliestDate = (values: Array<Date | null | undefined>) =>
  values.reduce<Date | null>((earliest, value) => {
    if (!value) {
      return earliest;
    }
...
const summarizeMaterialProgress = (
  fileProgresses: WorkspaceMaterialProgressSnapshot[],
  sourceFiles: OnboardingWorkspaceMaterialFileResponse[],
  stageStatus: string,
  fallbackDates: {
  ...
  const stageFinished =
    normalizedStageStatus === "WAITING_EXAM" ||
  ...
  const lastReadAt =
    getMostRecentDate(fileProgresses.map((item) => item.lastReadAt)) ?? null;
  const completedAt =
    getMostRecentDate(fileProgresses.map((item) => item.completedAt)) ??
    (stageFinished ? fallbackDates.stageCompletedAt : null);
  const openCount = fileProgresses.reduce(
    (sum, item) => sum + Number(item.openCount ?? 0),
    0
  );
...
const buildOnboardingNotificationContext = (params: {
  ...
  const cardNumber =
    params.employee.CardNo?.trim() || String(params.employee.UserId);
...
const buildSbuSubPicOnboardingNotificationContext = (params: {
  ...
  const employeeName =
    normalizeNote(params.employee.Name) ?? `Employee ${params.employee.UserId}`;
...
const groupSbuSubPicRecipientsByEmployee = (
  recipients: SbuSubPicOnboardingRecipient[]
) => {
  const grouped = new Map<number, SbuSubPicOnboardingRecipient[]>();
  ...
  try {
    ...
    for (const recipient of employeeRecipients) {
      ...
      for (const template of picNotificationTemplates) {
        ...
        if (channel === CHANNEL_WHATSAPP && !phoneNumber) {
          logger.warn(
            "Skipping SBU Sub PIC onboarding resume notification without phone",
            {
              onboardingAssignmentId: params.assignment.onboardingAssignmentId,
              onboardingDecisionId: params.onboardingDecisionId,
...
const buildEmployeeOnboardingPlacementMap = async (
  userIds: number[]
): Promise<Map<number, OnboardingPlacementInfo>> => {
  const normalizedUserIds = Array.from(
    new Set(
      userIds.filter((userId) => Number.isInteger(userId) && userId > 0)
    )
  ...
  const mutablePlacementMap = new Map<number, MutableOnboardingPlacementInfo>();
  const getMutablePlacement = (userId: number) => {
    ...
    const next = {
      sources: new Set<OnboardingPlacementSource>(),
      details: [],
  ...
  const addPlacementDetail = (
    userId: number | null | undefined,
    source: OnboardingPlacementSource,
    detail: Omit<EmployeeOnboardingPlacementDetailResponse, "source" | "label"> & {
      label?: string;
    }
  ) => {
  ...
  const pilarNameMap = new Map(
    pilarNames.map((pilar) => [pilar.id, normalizeNote(pilar.pilar_name)])
  );
  const sbuNameMap = new Map(
    sbuNames.map((sbu) => [sbu.id, normalizeNote(sbu.sbu_name)])
  );
    const sbuSubNameMap = new Map(
      sbuSubNames.map((sbuSub) => [
        sbuSub.id,
        normalizeNote(sbuSub.sbu_sub_name),
      ])
    );
    const sbuSubPicIds = Array.from(
      new Set(
        sbuSubNames
    ...
    const sbuSubPicEmployees =
      sbuSubPicIds.length > 0
        ? await prismaEmployee.em_employee.findMany({
            where: { UserId: { in: sbuSubPicIds } },
            select: { UserId: true, Name: true },
          })
        : [];
    const sbuSubPicEmployeeNameMap = new Map(
      sbuSubPicEmployees.map((employee) => [
        employee.UserId,
        normalizeNote(employee.Name),
      ])
    );
    const sbuSubPicNameMap = new Map(
      sbuSubNames.map((sbuSub) => [
        sbuSub.id,
        sbuSub.pic
  ...
  for (const member of chartMembers) {
    ...
    addPlacementDetail(
      member.userId,
      ONBOARDING_PLACEMENT_SOURCES.CHART_MEMBER,
      {
  ...
  for (const pilar of pilarPics) {
    addPlacementDetail(
      pilar.pic,
      ONBOARDING_PLACEMENT_SOURCES.PILAR_PIC,
      {
        roleLabel: "PIC Pilar",
        positionName: null,
        jabatanName: null,
  ...
  for (const sbu of sbuPics) {
    const pilarName =
      sbu.sbu_pilar !== null && sbu.sbu_pilar !== undefined
        ? pilarNameMap.get(sbu.sbu_pilar)
        : null;
    addPlacementDetail(
      sbu.pic,
      ONBOARDING_PLACEMENT_SOURCES.SBU_PIC,
      {
        roleLabel: "PIC SBU",
        positionName: null,
        jabatanName: null,
  ...
  for (const sbuSub of sbuSubPics) {
    const pilarName =
      sbuSub.sbu_pilar !== null && sbuSub.sbu_pilar !== undefined
        ? pilarNameMap.get(sbuSub.sbu_pilar)
        : null;
    const sbuName =
      sbuSub.sbu_id !== null && sbuSub.sbu_id !== undefined
        ? sbuNameMap.get(sbuSub.sbu_id)
        : null;
    addPlacementDetail(
      sbuSub.pic,
      ONBOARDING_PLACEMENT_SOURCES.SBU_SUB_PIC,
      {
...
const getOnboardingPlacementInfo = (
  placementMap: Map<number, OnboardingPlacementInfo>,
  userId: number
): OnboardingPlacementInfo =>
  placementMap.get(userId) ?? {
...
const toSummaryResponse = (
...
): EmployeeOnboardingSummaryResponse | null => {
  ...
  const hasActiveAssignment =
    assignment.isActive && !isFinalAssignmentStatus(assignment.status);
  ...
  return {
    ...
    hasActiveAssignment,
    canStart:
      placement.hasOnboardingPlacement &&
      !hasActiveAssignment &&
      !requiresDecision,
    requiresDecision,
...
const toNotStartedEmployeeSummaryResponse = (
  portalKey: string,
  employee: { UserId: number; Name: string | null },
  placementMap: Map<number, OnboardingPlacementInfo>
): EmployeeOnboardingSummaryResponse => {
...
const hasOnboardingPlacement = (
  placementMap: Map<number, OnboardingPlacementInfo>,
  userId: number
) => getOnboardingPlacementInfo(placementMap, userId).hasOnboardingPlacement;
...
export class OnboardingService {
  static async expireOverdueAssignments(options: {
    ...
    const now = new Date();
    const participantReferenceIds = Array.from(
      new Set(
        (options.participantReferenceIds ?? [])
          .map((value) => value.trim())
          .filter((value) => value.length > 0)
      )
    );
    ...
    for (const assignment of assignments) {
      ...
      const failureNote = `Masa tenggat onboarding berakhir pada ${formatIndonesianDate(
        assignment.dueAt
      )}. Status otomatis menjadi gagal dan menunggu keputusan HRD.`;
  ...
  static async decideOnboarding(
  ...
  ): Promise<DecideOnboardingResponse> {
    const validated = Validation.validate(
      OnboardingValidation.DECIDE_ONBOARDING,
      request
    ) as DecideOnboardingRequest;
    const decisionType = normalizeUpper(
      validated.decisionType
    ) as DecideOnboardingRequest["decisionType"];
    ...
    if (!canDecideAsPic) {
      throw new ResponseError(
        403,
        "Keputusan onboarding hanya bisa dibuat oleh HRD atau PIC terkait"
      );
    }
    ...
    if (
      ...
      throw new ResponseError(
        403,
        "PIC hanya bisa membekukan atau membatalkan status beku onboarding"
      );
    }
    ...
    if (decisionType === DECISION_FAIL_FINAL && !canDecideAsHrd) {
      throw new ResponseError(
        403,
        "Hanya HRD yang bisa menetapkan gagal onboarding final"
      );
    }
    ...
    await prismaFlowly.$transaction(async (tx) => {
      if (decisionType === DECISION_PASS_OVERRIDE) {
        ...
        const totalDurationDay = Math.max(
          1,
          Math.ceil(
            (nextDueAt.getTime() - assignment.startedAt.getTime()) /
              (24 * 60 * 60 * 1000)
          )
        );
  ...
  static async listMyWorkspace(
    requesterUserId: string
  ): Promise<MyOnboardingWorkspaceResponse> {
    ...
    const latestByPortal = new Map<string, (typeof assignments)[number]>();
    for (const assignment of assignments) {
    ...
    const scheduleIdToPortalKey = new Map<number, string>();
    const scheduleIdToStageTemplateId = new Map<number, string>();
    ...
    >();
    for (const assignment of assignments) {
      const portalKey = normalizePortalKey(
        assignment.portalTemplate?.portalKey ?? assignment.portalKey
      );
      for (const stageProgress of assignment.stageProgresses) {
        ...
        scheduleIdToStageTemplateId.set(
          scheduleId,
    ...
    const onboardingSchedules =
      onboardingScheduleIds.length > 0
        ...
        : [];
    const scheduleIdToScheduleName = new Map<number, string>();
    ...
    const scheduleTypeDurations =
      onboardingScheduleIds.length > 0
        ...
        : [];
    const stageTemplateDurationSeconds = new Map<string, number>();
    for (const row of scheduleTypeDurations) {
      ...
      stageTemplateDurationSeconds.set(
        stageTemplateId,
        (stageTemplateDurationSeconds.get(stageTemplateId) ?? 0) +
          durationMinutes * 60
      );
    }
    const certificateRows =
      Number.isInteger(employeeId) && employeeId > 0
        ...
        : [];
    const certificateScheduleIds = Array.from(
      new Set(
        certificateRows
    ...
    const missingCertificateScheduleIds = certificateScheduleIds.filter(
      (scheduleId) => !scheduleIdToScheduleName.has(scheduleId)
    );
    const certificateSchedules =
      missingCertificateScheduleIds.length > 0
        ? await prismaEmployee.em_schedule1.findMany({
            where: {
              Id: {
    ...
    const certificateTemplateIds = Array.from(
      new Set(certificateRows.map((certificate) => certificate.cert_templ_id))
    );
    const certificateTemplates =
      certificateTemplateIds.length > 0
        ...
        : [];
    const certificateTemplateMap = new Map(
      certificateTemplates.map((template) => [template.certificate_id, template] as const)
    );
    const fallbackCertificatePortalKey =
      Array.from(latestByPortal.keys())[0] ?? DEFAULT_PORTAL_KEY;
    const certificatesByPortal = new Map<
      string,
      OnboardingWorkspaceCertificateResponse[]
    >();
    ...
    const sourceMaterialMap = new Map(
      sourceMaterials.map((material) => [material.materialId, material] as const)
    );
    const workspaceExamIds = Array.from(
      new Set(
        assignments.flatMap((assignment) =>
          assignment.stageProgresses.flatMap((stageProgress) =>
            stageProgress.stageTemplate.stageExams.map((stageExam) => stageExam.examId)
          )
        )
      )
    );
    const [sourceExams, sourceExamQuestions, sourceQuestionTypes] =
      workspaceExamIds.length > 0
        ? await Promise.all([
        ...
        : [[], [], []] as const;
    const sourceExamNameMap = new Map(
      sourceExams.map((exam) => [
        exam.id,
        normalizeNote(exam.exam_name) ?? `Ujian ${exam.id}`,
      ] as const)
    );
    const sourceQuestionTypeMap = new Map(
      sourceQuestionTypes.map((type) => [
        type.Id,
        normalizeNote(type.TypeName) ?? `Tipe ${type.Id}`,
      ] as const)
    );
    const sourceQuestionsByExam = new Map<number, WorkspaceExamQuestionSnapshot[]>();
        ...
        assignments.flatMap((assignment) =>
          assignment.stageProgresses.flatMap((stageProgress) =>
            stageProgress.examAttempts
              ...
              .filter((examsId): examsId is string => Boolean(examsId))
          )
        )
      )
    );
    const sessionScoreRows =
      attemptSessionIds.length > 0
    ...
    const portals = Array.from(latestByPortal.values()).map((assignment) => ({
      ...
      portalKey: normalizePortalKey(
        assignment.portalTemplate?.portalKey ?? assignment.portalKey
      ),
      portalName:
        normalizeNote(assignment.portalTemplate?.portalName) ??
      ...
      certificates:
        certificatesByPortal.get(
          normalizePortalKey(assignment.portalTemplate?.portalKey ?? assignment.portalKey)
        ) ?? [],
      stages: assignment.stageProgresses.map((stageProgress) => {
        ...
        const resolveAttemptScore = (attempt: typeof latestExamAttempt) =>
          attempt?.score ??
          (attempt?.employeeExamSessionId
            ? sessionScoreByExamId.get(attempt.employeeExamSessionId) ?? null
            : null);
        const previousScoredAttemptScore =
          stageProgress.remedialCount > 0
            ? stageProgress.examAttempts
                .slice(1)
                .map((attempt) => resolveAttemptScore(attempt))
                .find((score): score is number => score != null) ?? null
            : null;
        const materialProgressMap = new Map(
          stageProgress.materialProgresses.map((progress) => [
            buildMaterialProgressKey(
              progress.onboardingStageMaterialId,
              progress.sourceFileId
            ),
            ...
            } satisfies WorkspaceMaterialProgressSnapshot,
          ])
        );
        ...
        return {
          ...
          materials: stageProgress.stageTemplate.stageMaterials.map((stageMaterial) => {
            ...
            const selectedFiles = filterWorkspaceSelectedFiles(
              sourceFiles,
              selectionMetadata
            ).map((file) => {
              const progress =
                materialProgressMap.get(
                  buildMaterialProgressKey(stageMaterial.onboardingStageMaterialId, file.id)
                ) ?? null;
            ...
            const fileSelectionMode: "ALL" | "SELECTED" =
              selectionMetadata.mode === "SELECTED" &&
              selectedFiles.length < sourceFiles.length
                ? "SELECTED"
                : "ALL";
            const materialLevelProgress =
              materialProgressMap.get(
                buildMaterialProgressKey(stageMaterial.onboardingStageMaterialId, 0)
              ) ?? null;
            const aggregateProgress = summarizeMaterialProgress(
            ...
            return {
              ...
              materialCode:
                normalizeNote(sourceMaterial?.materialCode) ??
                `MATERI-${stageMaterial.materiId}`,
              materialTitle:
                normalizeNote(sourceMaterial?.materialTitle) ??
                `Materi ${stageMaterial.materiId}`,
              ...
              readAt:
                aggregateProgress.readAt ??
                materialLevelProgress?.readAt ??
                null,
              lastReadAt:
                aggregateProgress.lastReadAt ??
                materialLevelProgress?.lastReadAt ??
                null,
              completedAt:
                aggregateProgress.completedAt ??
                materialLevelProgress?.completedAt ??
                null,
              files: selectedFiles,
              note:
                selectionMetadata.rawNote ??
                normalizeNote(sourceMaterial?.assignmentNote) ??
                null,
            };
          }),
          exams: buildWorkspaceExamSummaries(
            stageProgress.stageTemplate.stageExams,
            sourceExamNameMap,
            sourceQuestionsByExam,
            sourceQuestionTypeMap,
            stageTemplateDurationSeconds.get(stageProgress.onboardingStageTemplateId)
          ),
        };
      }),
    }));
  ...
  static async listAdminMonitoring(
    requesterUserId: string,
    options: OnboardingMonitoringOptions = {}
    ...
    const portalKeys =
      options.portalKeys?.map(normalizePortalKey).filter(Boolean) ??
    ...
    const portalTemplates = [...portalTemplatesRaw].sort(
      (left, right) =>
        getAdminPortalOrder(left.portalKey) - getAdminPortalOrder(right.portalKey)
    );

    const latestAssignmentsByParticipant = new Map<
      string,
      (typeof assignmentsRaw)[number]
    >();
    ...
    const employeeMap = new Map<number, AdminMonitoringEmployeeSnapshot>(
      employees.map((employee) => [
        employee.UserId,
        {
          ...
          email: normalizeEmail(employee.email),
          DeptId:
            Number.isInteger(employee.DeptId) && Number(employee.DeptId) > 0
    ...
    const attemptSessionIds = Array.from(
      new Set(
        latestAssignments.flatMap((assignment) =>
          assignment.stageProgresses.flatMap((stageProgress) =>
            stageProgress.examAttempts
              .map((attempt) => attempt.employeeExamSessionId)
              .filter((examsId): examsId is string => Boolean(examsId))
    ...
    const portals = portalTemplates.map((portalTemplate) => {
      ...
      const portalStageTemplates = portalTemplate.stageTemplates.map(
        (stageTemplate) => ({
      ...
      const participants = latestAssignments
        ...
        .map((assignment) => {
          ...
          const employee =
            Number.isInteger(numericParticipantId) && numericParticipantId > 0
              ? employeeMap.get(numericParticipantId) ?? null
              : null;

          const stageProgressByTemplateId = new Map(
            assignment.stageProgresses.map((stageProgress) => [
              stageProgress.onboardingStageTemplateId,
              stageProgress,
            ] as const)
          );
          ...
          const stages = portalTemplate.stageTemplates.map((stageTemplate) => {
            const stageProgress =
              stageProgressByTemplateId.get(
                stageTemplate.onboardingStageTemplateId
              ) ??
              assignment.stageProgresses.find(
                (item) => item.stageOrder === stageTemplate.stageOrder
              ) ??
              null;
            const stageStatus = stageProgress?.status ?? "LOCKED";
            const stageCompletedAt =
              stageProgress?.completedAt ?? stageProgress?.passedAt ?? null;
            const latestExamAttempt = stageProgress?.examAttempts[0] ?? null;
            const resolveAttemptScore = (
              attempt?: {
                score: number | null;
            ...
            const materials = stageTemplate.stageMaterials.map((stageMaterial) => {
              ...
              const selectionMetadata = parseWorkspaceSelectionMetadata(
                stageMaterial.note
              );
              const selectedFiles = filterWorkspaceSelectedFiles(
                sourceFiles,
              ...
              return {
                ...
                materialTitle:
                  normalizeNote(sourceMaterial?.materialTitle) ??
                  `Materi ${stageMaterial.materiId}`,
                materialDescription: normalizeNote(
                  sourceMaterial?.materialDescription
                ),
                isRequired: stageMaterial.isRequired,
                orderIndex: stageMaterial.orderIndex,
                totalFileCount: sourceFiles.length,
                ...
                selectedFileIds,
                fileSelectionMode,
                readFileCount: selectedFiles.filter(
                  (file) =>
                    Boolean(
                      file.readAt ||
                        file.lastReadAt ||
                ...
                status: aggregateProgress.status,
                readAt:
                  aggregateProgress.readAt ?? materialLevelProgress?.readAt ?? null,
                lastReadAt:
                  aggregateProgress.lastReadAt ??
                  materialLevelProgress?.lastReadAt ??
                  null,
            ...
            const totalMaterialCount = materials.reduce(
              (sum, material) => sum + Math.max(material.fileCount, 1),
              0
            );
            ...
            return {
              ...
              totalOpenCount: materials.reduce(
                (sum, material) => sum + Number(material.openCount ?? 0),
                0
              ),
              firstReadAt: getEarliestDate(
                materials.map((material) => material.readAt)
              ),
              lastReadAt: getMostRecentDate(
                materials.map((material) => material.lastReadAt)
              ),
              materials,
            };
          });
          ...
          const currentStage =
            ...
            stages.find(
              (stage) =>
                normalizeUpper(stage.status) !== "PASSED" &&
          ...
          return {
            ...
            totalMaterialCount: stages.reduce(
              (sum, stage) => sum + stage.totalMaterialCount,
              0
            ),
            readMaterialCount: stages.reduce(
              (sum, stage) => sum + stage.readMaterialCount,
              0
            ),
            totalOpenCount: stages.reduce(
              (sum, stage) => sum + stage.totalOpenCount,
              0
            ),
            firstReadAt: getEarliestDate(
              stages.map((stage) => stage.firstReadAt)
            ),
            lastReadAt: getMostRecentDate(
              stages.map((stage) => stage.lastReadAt)
            ),
            stages,
          };
        })
      ...
      return {
        ...
        activeParticipants: participants.filter(
          (participant) => !isFinalAssignmentStatus(participant.status)
        ).length,
        completedParticipants: participants.filter((participant) =>
          isCompletedAssignmentStatus(participant.status)
        ).length,
        totalOpenCount: participants.reduce(
          (sum, participant) => sum + Number(participant.totalOpenCount ?? 0),
          0
        ),
        firstReadAt: getEarliestDate(
          participants.map((participant) => participant.firstReadAt)
        ),
        lastReadAt: getMostRecentDate(
          participants.map((participant) => participant.lastReadAt)
        ),
  ...
  static async listPicMonitoring(
    requesterUserId: string
  ): Promise<AdminOnboardingMonitoringResponse> {
    const scope = await resolvePicOnboardingScope(requesterUserId);
    if (!scope) {
  ...
  static async startMaterialRead(
    requesterUserId: string,
    request: StartOnboardingMaterialReadRequest
  ): Promise<StartOnboardingMaterialReadResponse> {
    const participantReferenceId = requesterUserId.trim();
    if (!participantReferenceId) {
      throw new ResponseError(401, "Unauthorized");
    }
    ...
    const validated = Validation.validate(
      OnboardingValidation.START_MATERIAL_READ,
      request
    ) as StartOnboardingMaterialReadRequest;

    const sourceFileId = Number(validated.sourceFileId ?? 0);
    const normalizedSourceFileId =
      Number.isInteger(sourceFileId) && sourceFileId > 0 ? sourceFileId : 0;
    const actorId = trimAuditActor(requesterUserId);
    ...
    const sourceMaterialMap = new Map(
      (await OnboardingMaterialService.listSourceMaterials()).map((material) => [
        material.materialId,
        material,
      ] as const)
    );
    ...
    return prismaFlowly.$transaction(async (tx) => {
      ...
      if (stageStatus === "LOCKED") {
        throw new ResponseError(
          403,
          "Tahap onboarding ini belum aktif untuk dibaca"
        );
      }

      if (
        assignmentStatus === ASSIGNMENT_STATUS_TRANSFER_REVIEW ||
        ...
        throw new ResponseError(
          403,
          "Onboarding sudah terkunci karena dibekukan, gagal, atau sudah selesai"
        );
      }
      ...
      const activeExamSessionId = normalizeNote(
        activeExamAttempt?.employeeExamSessionId
      );
      if (activeExamSessionId) {
        ...
        if (
          ...
          throw new ResponseError(
            403,
            "Materi dikunci selama ujian sedang berlangsung"
          );
        }
      }
      ...
      const selectedSourceFileIds = new Set(
        resolveSelectedSourceFileIds(
          sourceFiles.map((file) => file.id),
          stageMaterial.note
        )
      );
      const sourceFile =
        normalizedSourceFileId > 0
      ...
      if (
        stageProgress.assignment.currentStageOrder === null ||
        ...
        ? await tx.onboardingMaterialProgress.update({
            where: {
              onboardingMaterialProgressId:
                existingProgress.onboardingMaterialProgressId,
            },
            data: {
              fileName: normalizeNote(validated.fileName) ?? existingProgress.fileName,
              fileTitle:
                normalizeNote(validated.fileTitle) ?? existingProgress.fileTitle,
              status:
                normalizeUpper(existingProgress.status) === "COMPLETED"
      ...
      const shouldMoveToExam =
        areStageMaterialsReadyForExam({
          stageMaterials,
          materialProgresses,
          sourceMaterialMap,
        }) &&
  ...
  static async listEmployeeSummary(
    requesterUserId: string,
    request: ListEmployeeOnboardingSummaryRequest
  ) {
    await ensureHrdReadAccess(requesterUserId);

    const validated = Validation.validate(
      OnboardingValidation.LIST_EMPLOYEE_SUMMARY,
      request
    ) as ListEmployeeOnboardingSummaryRequest;
    ...
    const summariesByEmployee = new Map<number, EmployeeOnboardingSummaryResponse>();
    ...
    for (const employee of employees) {
      if (!summariesByEmployee.has(employee.UserId)) {
        summariesByEmployee.set(
          employee.UserId,
          toNotStartedEmployeeSummaryResponse(portalKey, employee, placementMap)
        );
      }
    }
  ...
  static async startEmployees(
    requesterUserId: string,
    request: StartEmployeeOnboardingRequest
  ): Promise<StartEmployeeOnboardingResponse> {
    await ensureHrdCrudAccess(requesterUserId);

    const validated = Validation.validate(
      OnboardingValidation.START_EMPLOYEE,
      request
    ) as StartEmployeeOnboardingRequest;
    ...
    if (!portalTemplate) {
      throw new ResponseError(
        400,
        `Template onboarding portal ${portalKey} tidak ditemukan`
      );
    }
    ...
    if (stageTemplates.length === 0) {
      throw new ResponseError(
        400,
        `Template onboarding portal ${portalKey} belum memiliki tahap aktif`
      );
    }
    ...
    const employeeMap = new Map<number, EmployeeLookup>(
      employees.map((employee) => [
        employee.UserId,
        {
          UserId: employee.UserId,
    ...
    const skipped: EmployeeOnboardingSkipItem[] = [];
    ...
    const eligibleEmployees = employees.filter(
      (employee) => !hasMeaningfulDate(employee.ResignDate)
    );
    ...
    const placementMap = await buildEmployeeOnboardingPlacementMap(
      activeFreeEmployees.map((employee) => employee.UserId)
    );
    ...
    if (startableEmployees.length === 0) {
      ...
      const firstLoginParticipantRuntimeTemplates =
        await resolveRuntimeNotificationTemplates({
          portalKey,
          eventKey: OMS_FIRST_LOGIN_EVENT_KEY,
          recipientRole: PARTICIPANT_RECIPIENT_ROLE,
        });
      const returningParticipantRuntimeTemplates =
        await resolveRuntimeNotificationTemplates({
          portalKey,
          eventKey: ONBOARDING_STARTED_EVENT_KEY,
          recipientRole: PARTICIPANT_RECIPIENT_ROLE,
      ...
      const sbuSubPicRecipientsByEmployee =
        groupSbuSubPicRecipientsByEmployee(sbuSubPicRecipients);
      const sbuSubPicRuntimeTemplates =
        await resolveRuntimeNotificationTemplates({
          portalKey,
          eventKey: ONBOARDING_STARTED_EVENT_KEY,
          recipientRole: SBU_SUB_PIC_RECIPIENT_ROLE,
      ...
      const createNotificationOutboxId =
        ...
        (startableEmployees.some(
          (employee) => !isEmployeeFirstLogin(employee.isFirstLogin)
        ) &&
    ...
    const employeePasswordUpdates: Array<{
      userId: number;
      hashedPassword: string;
    }> = [];
    const assignmentsToCreate: Prisma.OnboardingAssignmentCreateManyInput[] = [];
    const stageProgressesToCreate: Prisma.OnboardingStageProgressCreateManyInput[] =
      [];
    const notificationOutboxesToCreate: Prisma.NotificationOutboxCreateManyInput[] =
      [];
    const started: EmployeeOnboardingStartItem[] = [];
    ...
    for (const employee of startableEmployees) {
      ...
      assignmentsToCreate.push({
        ...
        const employeeForNotification: EmployeeLookup = {
          UserId: employee.UserId,
          CardNo: employee.CardNo ?? null,
          Name: employee.Name ?? null,
          Phone: employee.Phone ?? null,
        ...
        const participantNotificationTemplates: NotificationTemplateForOutbox[] =
          isFirstLoginEmployee
        ...
        notificationOutboxesToCreate.push(
          ...participantNotificationTemplates.map((template) => ({
            notificationOutboxId: createNotificationOutboxId(),
            notificationTemplateId: template.notificationTemplateId,
            portalKey,
            ...
            phoneNumber:
              template.channel === CHANNEL_WHATSAPP ? phoneNumber ?? "" : "",
            message: trimMessage(
        ...
        const sbuSubPicEmployeeRecipients =
          sbuSubPicRecipientsByEmployee.get(employee.UserId) ?? [];
        if (
          createNotificationOutboxId &&
          ...
          for (const recipient of sbuSubPicEmployeeRecipients) {
            ...
            for (const template of sbuSubPicNotificationTemplates) {
              ...
              if (channel === CHANNEL_WHATSAPP && !phoneNumber) {
                logger.warn(
                  "Skipping SBU Sub PIC onboarding notification without phone",
                  {
                    onboardingAssignmentId,
                    employeeUserId: employee.UserId,
                    recipientUserId: recipient.recipientUserId,
        ...
        stageProgressesToCreate.push(
        ...stageTemplates.map((stageTemplate, index) => ({
