...
export class AuditLogService {
  static async list(requesterId: string, reqQuery: ListAuditLogRequest): Promise<AuditLogListResponse> {
    ...
    if (request.dateFrom || request.dateTo) {
      ...
      if (request.dateFrom) whereClause.createdAt.gte = request.dateFrom;
      if (request.dateTo) whereClause.createdAt.lte = request.dateTo;
    }
    ...
    const data: AuditLogResponse[] = rows.map((row) => ({
      ...
      changes: parseJson<Record<string, unknown>[]>(row.changes),
