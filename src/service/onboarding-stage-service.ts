...
type CustomerProgramAccessRow = {
  pretail: string | null;
  indomata: string | null;
};

type CustomerProgramAccess = {
  pretail: string | null;
  indomata: string | null;
  bypassProgramFilter: boolean;
  canAccessPos: boolean;
  canAccessIndomata: boolean;
};
...
type CustomerLearningFileAccessRequest = CustomerLearningStagesRequest &
  CustomerLearningFileOpenRequest;

type CustomerLearningSourceFile = Awaited<
...
const PORTAL_LEARNING_KEYS = new Set<string>(["CUSTOMER", "SUPPLIER"]);
...
const normalizePortalLearningKey = (
  value: string | null | undefined,
  fallback = CUSTOMER_PORTAL_KEY
) => {
...
const resolveLearningParticipant = (
  ...
  const participantReferenceId =
    normalizeOptionalText(request.participantReferenceId) ??
    (portalKey === CUSTOMER_PORTAL_KEY
      ? normalizeOptionalText(request.custId)
      : null);
  const participantReferenceType =
    normalizeOptionalText(request.participantReferenceType) ?? portalKey;
...
const normalizeSourceFileId = (value: number | string | null | undefined) => {
  const numeric = Number(value ?? 0);
  return Number.isInteger(numeric) && numeric > 0 ? numeric : 0;
};

const deriveMaterialStatus = (files: CustomerLearningMaterialFileResponse[]) => {
...
const isCustomerProgramEnabled = (value: string | null | undefined) => {
  const normalized = normalizeOptionalText(value);
  return Boolean(normalized && normalizeUpper(normalized) !== "X");
};

const getCustomerProgramAccess = async (
  custId: string | null | undefined
): Promise<CustomerProgramAccess> => {
  ...
  const rows = await prismaOptidom.$queryRaw<CustomerProgramAccessRow[]>`
    SELECT DISTINCT
      [PRetail] AS pretail,
  ...
  const indomataValues = rows.map((row) =>
    normalizeOptionalText(row.indomata)
  );
  const pretail =
    pretailValues.find(isCustomerProgramEnabled) ??
    pretailValues.find((value) => Boolean(value)) ??
    null;
  const indomata =
    indomataValues.find(isCustomerProgramEnabled) ??
...
const canAccessCustomerLearningStage = (
  stageOrder: number,
  access: CustomerProgramAccess
) => {
...
const parseSelectedMaterialFileIds = (note: string | null | undefined) => {
...
const filterCustomerMaterialFiles = <
  TFile extends {
    id: number;
  }
>(
  files: TFile[],
  selectedFileIds: Set<number> | null
) => {
...
type CustomerLearningRuntimeStageProgress = {
...
type CustomerLearningRuntime = {
  onboardingAssignmentId: string;
  stageProgressByTemplateId: Map<string, CustomerLearningRuntimeStageProgress>;
  stageProgressByOrder: Map<number, CustomerLearningRuntimeStageProgress>;
  materialProgressByKey: Map<
    string,
    CustomerLearningRuntimeStageProgress["materialProgresses"][number]
  >;
};
...
const buildMaterialProgressKey = (
  ...
  `${onboardingStageProgressId}:${onboardingStageMaterialId}:${sourceFileId}`;

const ensureCustomerLearningSourceFile = async (
  request: CustomerLearningFileOpenRequest,
  stageMaterial: {
    onboardingStageMaterialId: string;
    onboardingStageTemplateId: string;
    materiId: number;
...
): Promise<CustomerLearningSourceFile> => {
  ...
  const sourceMaterial = sourceMaterials.find(
    (material) => material.materialId === stageMaterial.materiId
  );
  if (!sourceMaterial) {
  ...
  const sourceFile = filterCustomerMaterialFiles(
    sourceMaterial.files,
    selectedFileIds
  ).find((file) => file.id === sourceFileId);
...
const toCustomerLearningRuntime = (assignment: {
  onboardingAssignmentId: string;
  stageProgresses: CustomerLearningRuntimeStageProgress[];
}): CustomerLearningRuntime => {
  const stageProgressByTemplateId = new Map<
    string,
    CustomerLearningRuntimeStageProgress
  >();
  ...
  for (const stageProgress of assignment.stageProgresses) {
    ...
    for (const materialProgress of stageProgress.materialProgresses) {
      materialProgressByKey.set(
        buildMaterialProgressKey(
          stageProgress.onboardingStageProgressId,
          materialProgress.onboardingStageMaterialId,
          materialProgress.sourceFileId
        ),
        materialProgress
      );
    }
...
const ensureCustomerLearningRuntime = async (params: {
  participantReferenceId: string | null | undefined;
  ...
  const participantReferenceType =
    normalizeOptionalText(params.participantReferenceType) ?? portalKey;
...
export class OnboardingStageService {
  static async list(requesterId: string): Promise<ListOnboardingStagesResponse> {
  ...
  static async listCustomerLearningStages(
    ...
    const {
      ...
      participantReferenceId,
      participantReferenceType,
    } = resolveLearningParticipant(request);
    ...
    const accessibleStageTemplates = portalTemplate.stageTemplates.filter(
      (stageTemplate) =>
        canAccessCustomerLearningStage(
          stageTemplate.stageOrder,
          customerProgramAccess
        )
    );
    ...
    const stages = accessibleStageTemplates
      .map((stageTemplate) => {
        ...
        const materials = stageTemplate.stageMaterials.map((stageMaterial) => {
          ...
          const files = filterCustomerMaterialFiles(
            sourceMaterial?.files ?? [],
            selectedFileIds
          ).map<CustomerLearningMaterialFileResponse>((file) => {
            const materialProgress = stageProgress
              ? runtime?.materialProgressByKey.get(
                  buildMaterialProgressKey(
                    stageProgress.onboardingStageProgressId,
                    stageMaterial.onboardingStageMaterialId,
                    normalizeSourceFileId(file.id)
                  )
                ) ?? null
              : null;
          ...
          return {
            ...
            materialCode:
              normalizeOptionalText(sourceMaterial?.materialCode) ??
              `MATERI-${stageMaterial.materiId}`,
            materialTitle:
              normalizeOptionalText(sourceMaterial?.materialTitle) ??
              `Materi ${stageMaterial.materiId}`,
            materialDescription: normalizeOptionalText(
            ...
            completedAt:
              status === "COMPLETED"
                ...
                : null,
            openCount: files.reduce(
              (sum, file) => sum + Number(file.openCount ?? 0),
              0
            ),
            files,
          };
        ...
        return {
          ...
          stageCode: stageTemplate.stageCode,
          stageName: stageTemplate.stageName,
          stageDescription: normalizeOptionalText(
            stageTemplate.stageDescription
          ),
          materialCount: materials.length,
          firstMaterialUrl:
            materials.find((material) => Boolean(material.firstFileUrl))
              ?.firstFileUrl ?? null,
          materials,
        };
      });
  ...
  static async authorizeCustomerLearningFileAccess(
    ...
    if (request.bypassProgramFilter) {
      ...
      return {
        ...
        onboardingStageMaterialId: stageMaterial.onboardingStageMaterialId,
        materialId: stageMaterial.materiId,
        sourceFile: await ensureCustomerLearningSourceFile(
          request,
          stageMaterial
        ),
      };
    ...
    const onboardingAssignmentId = normalizeOptionalText(
      request.onboardingAssignmentId
    );
    ...
    if (
      normalizeUpper(stageProgress.assignment.portalKey) !== portalKey ||
    ...
    if (portalKey === CUSTOMER_PORTAL_KEY) {
      const customerProgramAccess = await getCustomerProgramAccess(
        participantReferenceId
      );
      if (
        !canAccessCustomerLearningStage(
          stageProgress.stageOrder,
          customerProgramAccess
        )
      ) {
        throw new ResponseError(403, "Anda tidak memiliki akses ke materi ini");
  ...
  static async recordCustomerLearningFileOpen(
    ...
    request: CustomerLearningFileOpenRequest
  ): Promise<CustomerLearningFileOpenResponse> {
    const portalKey = normalizePortalLearningKey(request.portalKey);
    const participantReferenceType =
      normalizeOptionalText(request.participantReferenceType) ?? portalKey;
    ...
    const now = new Date();
    const actorId = toAuditActor(participantReferenceId);
    const createMaterialProgressId =
      await generateOnboardingMaterialProgressId();
    ...
    return prismaFlowly.$transaction(async (tx) => {
      ...
      if (
        normalizeUpper(stageProgress.assignment.portalKey) !==
          portalKey ||
        normalizeUpper(stageProgress.assignment.participantReferenceType) !==
          participantReferenceType ||
        stageProgress.assignment.participantReferenceId !== participantReferenceId ||
      ...
      if (
        ...
        await tx.onboardingAssignment.update({
          where: {
            onboardingAssignmentId:
              stageProgress.assignment.onboardingAssignmentId,
          },
          data: {
      ...
      const materialProgress = existingProgress
        ? await tx.onboardingMaterialProgress.update({
            ...
            data: {
              fileName:
                normalizeOptionalText(request.fileName) ??
                existingProgress.fileName,
              fileTitle:
                normalizeOptionalText(request.fileTitle) ??
      ...
      return {
        tracked: true,
        onboardingMaterialProgressId:
          materialProgress.onboardingMaterialProgressId,
        onboardingAssignmentId: stageProgress.onboardingAssignmentId,
        onboardingStageProgressId: stageProgress.onboardingStageProgressId,
        onboardingStageMaterialId: stageMaterial.onboardingStageMaterialId,
        sourceFileId,
  ...
  static async create(
    requesterId: string,
    request: CreateOnboardingStageRequest
  ) {
    await ensureAdminAccess(requesterId);
    ...
    await OnboardingEmployeeScheduleSyncService.syncStageByTemplateId(
      created.onboardingStageTemplateId
    );
  ...
  static async update(
    requesterId: string,
    request: UpdateOnboardingStageRequest
  ) {
    await ensureAdminAccess(requesterId);
    const validated = Validation.validate(
      OnboardingStageValidation.UPDATE_STAGE,
      request
    ) as UpdateOnboardingStageRequest;
    ...
    if (
      ...
      throw new ResponseError(
        400,
        "Tahap sudah dipakai peserta onboarding, tidak bisa dinonaktifkan"
      );
    }
    ...
    await OnboardingEmployeeScheduleSyncService.syncStageByTemplateId(
      existing.onboardingStageTemplateId
    );
  ...
  static async delete(
    requesterId: string,
    request: DeleteOnboardingStageRequest
  ) {
    await ensureAdminAccess(requesterId);
    const validated = Validation.validate(
      OnboardingStageValidation.DELETE_STAGE,
      request
    ) as DeleteOnboardingStageRequest;
    ...
    if (existing._count.stageProgresses > 0) {
      throw new ResponseError(
        400,
        "Tahap sudah dipakai peserta onboarding, tidak bisa dihapus"
      );
    }
