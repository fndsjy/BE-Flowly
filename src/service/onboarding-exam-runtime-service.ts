...
const normalizeTestAutofillScore = (value: number | string | null | undefined) => {
  if (value == null || value === "") {
...
const getNextAttemptNo = async (onboardingStageProgressId: string) => {
...
const parseFocusWarningCount = (note: string | null | undefined) => {
  const matches = Array.from(
    note?.matchAll(/Exam focus warnings:\s*(\d+)/gi) ?? []
  );
...
const resolveSelectedSourceFileIds = (
  ...
  const filtered = sourceFileIds.filter((sourceFileId) =>
    selectedSet.has(sourceFileId)
  );
  return filtered.length > 0 ? filtered : sourceFileIds;
};

const hasMaterialReadSignal = (progress: {
  status: string;
  readAt: Date | null;
...
const normalizeAnswerValue = (value: string | null | undefined) => {
  const text = (stripHtml(value) ?? normalizeOptionalText(value))?.replace(
    /#/g,
    " "
  );
  if (!text) {
...
const expandBooleanAnswerCandidates = (value: string | null | undefined) => {
...
const toLegacyAnswerText = (value: string | null | undefined) => {
...
const findSelectedOption = (
  ...
  return (
    question.em_questions2.find((item) => {
...
const sanitizeBadgeNumber = (value: string | null | undefined, fallback: number) => {
...
const toLegacyQuestionType = (
  typeId: number | null | undefined,
  typeMap: Map<number, string | null>
): SoalUrutItem["tipe"] => {
...
const toRuntimeQuestionType = (type: SoalUrutItem["tipe"]): RuntimeQuestionType => {
  if (type === "es") return "essay";
  if (type === "tf") return "boolean";
  return "mcq";
};

const parseSoalUrut = (value: string | null | undefined): SoalUrutItem[] => {
  const normalized = normalizeOptionalText(value);
  if (!normalized) {
    return [];
  }
  ...
  try {
    ...
    return parsed
      .map((item) => {
        ...
        const tipe = (item as { tipe?: unknown }).tipe;
        if (!Number.isInteger(questionId) || questionId <= 0) {
          return null;
        }

        const parsedOptions = Array.isArray((item as { options?: unknown }).options)
          ? Array.from(
              new Set(
                ((item as { options?: unknown }).options as unknown[])
                  .map((option) =>
                    typeof option === "string" ? option.trim() : null
                  )
                  .filter((option): option is string => Boolean(option))
              )
            )
          : [];
        const parsedItem: SoalUrutItem = {
          id: questionId,
      ...
      .filter((item): item is SoalUrutItem => Boolean(item));
  } catch {
    return [];
  }
};
...
const hasSameOrder = <T>(
  items: T[],
  previousIds: number[],
  getId: (item: T) => number
) =>
  items.length === previousIds.length &&
  items.every((item, index) => getId(item) === previousIds[index]);

const hasSameQuestionSet = <T>(
  items: T[],
  previousIds: number[],
  getId: (item: T) => number
) => {
...
const rotateOnce = <T>(items: T[]) =>
  items.length > 1 ? [...items.slice(1), items[0] as T] : items;

const shuffleDifferentFromPrevious = <T>(
  items: T[],
  previousIds: number[],
  getId: (item: T) => number
) => {
...
const encryptTokenPayload = (payload: string) => {
...
const getQuestionTypeMap = async () => {
  ...
  return new Map(rows.map((row) => [row.Id, row.TypeName] as const));
};

const buildQuestionsResponse = async (soalUrut: SoalUrutItem[]) => {
  ...
  return soalUrut
    .map((item, index) => {
      ...
      const optionMap = new Map(
        question.em_questions2.map((option) => [option.option_choices, option] as const)
      );
      const orderedOptionRows =
        item.options && item.options.length > 0
          ? [
              ...item.options
                .map((optionChoice) => optionMap.get(optionChoice))
                .filter((option): option is NonNullable<typeof option> => Boolean(option)),
              ...question.em_questions2.filter(
                (option) => !item.options?.includes(option.option_choices)
              ),
            ]
          : question.em_questions2;
      const options =
        item.tipe === "tf" && orderedOptionRows.length === 0
          ? TRUE_FALSE_OPTIONS
...
const getSessionDurationSeconds = (session: {
  ...
  if (session.start_time && session.is_token_expr) {
    const diff = Math.ceil(
      (session.is_token_expr.getTime() - session.start_time.getTime()) / 1000
    );
    if (diff > 0) {
      return diff;
    }
...
const activeRuntimeExamStarts = new Map<
  string,
  Promise<Awaited<ReturnType<typeof buildStartResponse>>>
>();

const createExamsId = async (timestamp: number, badgeNumber: string) => {
...
const ensureScheduleParticipant = async (scheduleId: number, employeeId: number) => {
...
const createOrUpdateAttempt = async (params: {
...
}): Promise<{ onboardingExamAttemptId: string | null; created: boolean }> => {
  ...
  for (let retry = 0; retry < 3; retry += 1) {
    ...
    const attemptNo = await getNextAttemptNo(
      params.stageProgress.onboardingStageProgressId
    );
...
const getAuthorizedStageProgress = async (
  requesterUserId: string,
  onboardingStageProgressId: string
) => {
...
const enqueueExamMonitorNotification = async (params: {
  ...
  const eventKey =
    params.event === "STARTED" ? EXAM_STARTED_EVENT_KEY : EXAM_FINISHED_EVENT_KEY;
  ...
  try {
    ...
    const recipientIds = EXAM_MONITOR_EMPLOYEE_IDS.filter(
    ...
    const notifiableRecipients = availableRecipients.filter(
      (recipient) => Boolean(recipient.phoneNumber)
    );
...
const buildMaterialProgressKey = (
  ...
  sourceFileId: number
) => `${onboardingStageMaterialId}:${sourceFileId}`;

const areAllStageMaterialsOpened = async (
  stageProgress: AuthorizedStageProgress
) => {
  const sourceMaterialMap = new Map<number, RuntimeSourceMaterial>(
...
const buildObjectiveScore = (params: {
  ...
  const correctCandidates = [
    ...
    .flatMap(normalizeCandidates)
    .filter((value, index, values) => values.indexOf(value) === index);
  const isCorrect = answerCandidates.some((candidate) =>
    correctCandidates.includes(candidate)
  );

  return isCorrect ? Number(params.question.score ?? 0) : 0;
};

const getAdminStageProgressForTestAutofill = async (
  onboardingStageProgressId: string
) => {
  const stageProgress = await prismaFlowly.onboardingStageProgress.findFirst({
    where: {
...
const getLatestTestAutofillSession = async (
  onboardingStageProgressId: string,
  employeeId: number
) => {
  const attempt = await prismaFlowly.onboardingExamAttempt.findFirst({
...
const buildTestAutofillAnswer = (params: {
  ...
  if (params.item.tipe === "tf") {
    ...
    if (
      correctCandidates.some((candidate) =>
        ["true", "benar", "yes", "ya", "1"].includes(candidate)
      )
    ) {
      return "True";
    }
    ...
    if (
      correctCandidates.some((candidate) =>
        ["false", "salah", "no", "tidak", "0"].includes(candidate)
      )
    ) {
      return "False";
    }
  ...
  const matchedOption = question.em_questions2.find((option) => {
    ...
    return (
      (choice ? correctCandidateSet.has(choice) : false) ||
      (text ? correctCandidateSet.has(text) : false)
    );
  });

  return (
    matchedOption?.option_choices ??
...
export class OnboardingExamRuntimeService {
  ...
  private static async startUnlocked(
    ...
    const previousQuestionIdsByType = new Map<SoalUrutItem["tipe"], number[]>();
    ...
    const groupedQuestions = new Map<number, typeof scheduleQuestions>();
    for (const question of scheduleQuestions) {
      const tipeSoal =
        question.tipeSoal != null && Number.isInteger(question.tipeSoal)
          ? question.tipeSoal
          : 0;
    ...
    const orderedShuffledQuestions = Array.from(groupedQuestions.entries())
      .sort((left, right) => {
        const leftFirstOrder = Math.min(
          ...left[1].map((question) => Number(question.urutanSoal ?? 999999))
        );
        const rightFirstOrder = Math.min(
          ...right[1].map((question) => Number(question.urutanSoal ?? 999999))
        );
      ...
      .flatMap(([tipeSoal, questions]) => {
        ...
        return shuffleDifferentFromPrevious(
          sortedQuestions,
          previousQuestionIdsByType.get(legacyType) ?? [],
          (question) => question.soalId
        );
      });
    const baseSoalUrut: SoalUrutItem[] = orderedShuffledQuestions.map((question) => ({
      id: question.soalId,
    ...
    const multipleChoiceQuestionIds = baseSoalUrut
      ...
      .map((item) => item.id);
    const optionRows =
      multipleChoiceQuestionIds.length > 0
    ...
    const soalUrut: SoalUrutItem[] = baseSoalUrut.map((item) => ({
      ...item,
    ...
    for (let attempt = 0; attempt < 5; attempt += 1) {
      const { examsId: candidateExamsId, timestamp: tokenTimestamp } =
        await createExamsId(timestamp + attempt, badgeNumber);
      ...
      try {
        ...
        const concurrentSession = await getExistingActiveSession(
          syncResult.scheduleId,
          employeeId
        );
  ...
  static async saveAnswer(
    requesterUserId: string,
    request: SaveRuntimeExamAnswerRequest
  ) {
  ...
  static async recordWarning(
    requesterUserId: string,
    request: RuntimeExamWarningRequest
  ) {
    const examsId = normalizeOptionalText(request.examsId);
    if (!examsId) {
      throw new ResponseError(400, "Sesi ujian wajib diisi");
    ...
    const warningCount = Math.max(
      existingCount + 1,
      Number.isInteger(requestedCount) && requestedCount > 0 ? requestedCount : 0
    );
  ...
  static async testAutofill(
    requesterUserId: string,
    request: TestAutofillRuntimeExamRequest
  ) {
    await ensureEmployeeAdminAccess(
      requesterUserId,
      "Hanya admin yang dapat menjalankan autofill testing onboarding"
    );
    ...
    if (allowedEmployeeIds.size === 0) {
      throw new ResponseError(
        403,
        `Autofill testing belum dikonfigurasi di ${TEST_AUTOFILL_EMPLOYEE_IDS_ENV}`
      );
    }

    const stageProgress = await getAdminStageProgressForTestAutofill(
      onboardingStageProgressId
    );
    ...
    if (!allowedEmployeeIds.has(employeeId)) {
      throw new ResponseError(
        403,
        "Employee ini tidak termasuk whitelist autofill testing onboarding"
      );
    }
    ...
    let target = await getLatestTestAutofillSession(
      onboardingStageProgressId,
      employeeId
    );
    ...
    if (!target) {
      ...
      target = await getLatestTestAutofillSession(
        onboardingStageProgressId,
        employeeId
      );
    }
    ...
    await prismaEmployee.$transaction(async (tx) => {
      ...
      await tx.em_session_exams.update({
        ...
        data: {
          ...
          is_correct: 0,
          is_notes:
            "Autofill testing oleh admin. Skor dummy menunggu release admin.",
    ...
    return {
      ...
      totalQuestionCount,
      message:
        "Jawaban testing berhasil dibuat dan menunggu release admin",
    };
  }

  static async submit(requesterUserId: string, request: SubmitRuntimeExamRequest) {
    const examsId = normalizeOptionalText(request.examsId);
    if (!examsId) {
      throw new ResponseError(400, "Sesi ujian wajib diisi");
    }
    ...
    const answerMap = new Map(
      (request.answers ?? [])
        ...
        .map((answer) => [answer.questionId, answer.answer] as const)
    );
    if (!request.autoSubmit) {
      ...
      if (missingQuestions.length > 0) {
        throw new ResponseError(
          400,
          `Lengkapi semua jawaban sebelum submit. Masih ada ${missingQuestions.length} soal belum diisi.`
        );
      }
    }
    const questions = await prismaEmployee.em_questions1.findMany({
