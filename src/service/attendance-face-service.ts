...
import {
  type DeleteFaceProfileRequest,
  type EnrollFaceProfileRequest,
  type FaceAttendanceEmployeeResponse,
  type FaceAttendanceEventType,
  type FaceScanLogResponse,
  type MatchFaceProfileRequest,
  type MatchFaceProfileResponse,
  type RecordFaceScanFailureRequest,
  type RecordFaceScanSuccessRequest,
  type RecordFaceScanSuccessResponse,
} from "../model/attendance-face-model.js";
...
type FaceProfileIdRow = {
  employeeFaceProfileId: bigint | number;
};

type FaceScanLogIdRow = {
  faceScanLogId: bigint | number;
};
...
type ScanLogRow = {
  ...
  livenessPassed: boolean | number;
  spoofType: string | null;
  failureReason: string | null;
};

type FaceProfileMatchRow = {
  userId: number;
  badgeNum: string;
  name: string | null;
  employeeFaceProfileId: bigint | number;
...
const parseDescriptor = (value: string | null): number[] | null => {
  if (!value) {
    return null;
  }
...
const toEmployeeResponse = (row: PublicEmployeeRow): FaceAttendanceEmployeeResponse => ({
  badgeNum: row.badgeNum,
  name: row.name ?? null,
});

const toScanLogResponse = (row: ScanLogRow): FaceScanLogResponse => ({
  faceScanLogId: String(row.faceScanLogId),
  employeeName: row.employeeName ?? null,
  badgeNum: row.badgeNum ?? null,
  eventType: row.scanEventType as FaceAttendanceEventType,
  scanStatus: row.scanStatus as FaceScanLogResponse["scanStatus"],
  scanTime: row.scanTime,
  matchConfidence: row.matchConfidence ?? null,
  livenessPassed: Boolean(row.livenessPassed),
  spoofType: row.spoofType as FaceScanLogResponse["spoofType"],
...
const getTodayBounds = (value: Date) => {
...
export class AttendanceFaceService {
  private static async getActiveEmployeeOrThrow(userId: number) {
    const rows = await prismaEmployee.$queryRaw<EmployeeStatusRow[]>`
      SELECT TOP 1
        [UserId] AS [userId],
  ...
  private static async getActiveEmployeeByBadgeOrThrow(badgeNum: string) {
  ...
  private static async getScanLogById(faceScanLogId: bigint | number) {
    const rows = await prismaEmployee.$queryRaw<ScanLogRow[]>`
      SELECT TOP 1
  ...
  private static async findBestFaceProfileMatch(
    faceDescriptor: number[]
  ): Promise<FaceProfileMatch> {
    const rows = await prismaEmployee.$queryRaw<FaceProfileMatchRow[]>`
      SELECT
    ...
    let bestMatch: FaceProfileMatch | null = null;
  ...
  static async listEmployees() {
    const rows = await prismaEmployee.$queryRaw<PublicEmployeeRow[]>`
      SELECT
        emp.[BadgeNum] AS [badgeNum],
        emp.[Name] AS [name]
      FROM [dbo].[em_employee] emp
  ...
  static async enroll(request: EnrollFaceProfileRequest) {
    const validated = Validation.validate(
      AttendanceFaceValidation.ENROLL,
      request
    ) as EnrollFaceProfileRequest;
    ...
    const createdRows = await prismaEmployee.$transaction(async (tx) => {
      ...
      return tx.$queryRaw<FaceProfileIdRow[]>`
  ...
  static async deleteProfile(request: DeleteFaceProfileRequest) {
    const validated = Validation.validate(
      AttendanceFaceValidation.DELETE_PROFILE,
      request
    ) as DeleteFaceProfileRequest;
  ...
  static async matchProfile(
    request: MatchFaceProfileRequest
  ): Promise<MatchFaceProfileResponse> {
    const validated = Validation.validate(
      AttendanceFaceValidation.MATCH_PROFILE,
      request
    ) as MatchFaceProfileRequest;
  ...
  static async recordSuccess(
    request: RecordFaceScanSuccessRequest
  ): Promise<RecordFaceScanSuccessResponse> {
    const validated = Validation.validate(
      AttendanceFaceValidation.SCAN_SUCCESS,
      request
    ) as RecordFaceScanSuccessRequest;
    ...
    const result = await prismaEmployee.$transaction(async (tx) => {
      const openAttendances = await tx.$queryRaw<OpenAttendanceRow[]>`
      ...
      let employeeAttendanceId: number;
      let scanEventType: Extract<FaceAttendanceEventType, "CHECK_IN" | "CHECK_OUT">;
      ...
      if (openAttendances[0]) {
        ...
        const attendanceRows = await tx.$queryRaw<OpenAttendanceRow[]>`
      ...
      const logRows = await tx.$queryRaw<FaceScanLogIdRow[]>`
  ...
  static async recordFailure(request: RecordFaceScanFailureRequest) {
    const validated = Validation.validate(
      AttendanceFaceValidation.SCAN_FAILURE,
      request
    ) as RecordFaceScanFailureRequest;

    const scanTime = new Date();

    const scanEventType: FaceAttendanceEventType =
      validated.scanStatus === "SPOOF" ? "SPOOF" : "FAILED";

    const rows = await prismaEmployee.$queryRaw<FaceScanLogIdRow[]>`
      INSERT INTO [dbo].[att_face_scan_logs] (
        [employee_user_id],
        [employee_face_profile_id],
        [scan_event_type],
  ...
  static async listLogs(limit: number) {
