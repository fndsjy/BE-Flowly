...
import {
  ...
  type DeleteCaseNotificationTemplateRequest,
  toCaseNotificationTemplateResponse,
  toCaseNotificationTemplateListResponse,
} from "../model/case-notification-template-model.js";
...
const normalizeCaseType = (value?: string | null) => {
  if (value === undefined || value === null) return null;
  const trimmed = value.trim();
  return trimmed.length > 0 ? trimmed.toUpperCase() : null;
};
...
export class CaseNotificationTemplateService {
  static async create(
    requesterId: string,
    reqBody: CreateCaseNotificationTemplateRequest
  ) {
    const request = Validation.validate(
      CaseNotificationTemplateValidation.CREATE,
      reqBody
    );
    ...
    const client = prismaFlowly as typeof prismaFlowly & {
      caseNotificationTemplate: {
        findFirst: (args: any) => Promise<{ caseNotificationTemplateId: string } | null>;
  ...
  static async update(
    requesterId: string,
    reqBody: UpdateCaseNotificationTemplateRequest
  ) {
    const request = Validation.validate(
      CaseNotificationTemplateValidation.UPDATE,
      reqBody
    );
    ...
    if (conflict) {
      throw new ResponseError(
        409,
        "Template with the same channel, role, action, and case type already exists"
      );
    }
  ...
  static async softDelete(
    requesterId: string,
    reqBody: DeleteCaseNotificationTemplateRequest
  ) {
    const request = Validation.validate(
      CaseNotificationTemplateValidation.DELETE,
      reqBody
    );
