...
import {
  type CreatePortalMenuMapRequest,
  type DeletePortalMenuMapRequest,
  type UpdatePortalMenuMapRequest,
  toPortalMenuMapListResponse,
  toPortalMenuMapResponse,
...
type ManagedResourceType = "PORTAL" | "MENU";
...
const resolveMasterAccessRole = async ({
  expectedType,
  masAccessId,
  resourceKey,
}: {
  expectedType: ManagedResourceType;
  masAccessId: string | null | undefined;
  ...
  if (!normalizedId && !normalizedKey) {
    throw new ResponseError(
      400,
      `${expectedType.toLowerCase()}MasAccessId or ${expectedType.toLowerCase()}Key is required`
    );
  }
    ...
    : await prismaFlowly.masterAccessRole.findUnique({
        where: {
          resourceType_resourceKey: {
            ...
            resourceKey: normalizedKey as string,
          },
        },
        select: masterAccessRoleSelect,
      });
  ...
  if (!record || record.isDeleted) {
    throw new ResponseError(
      400,
      `${expectedType === "PORTAL" ? "Portal" : "Menu"} resource not found`
    );
  }
  ...
  if (normalizedType !== expectedType) {
    throw new ResponseError(
      400,
      `${record.resourceKey} is not a ${expectedType.toLowerCase()} resource`
    );
  }
  ...
  if (normalizedKey && record.resourceKey !== normalizedKey) {
    throw new ResponseError(
      400,
      `${expectedType === "PORTAL" ? "Portal" : "Menu"} key does not match the selected resource`
    );
  }
...
const getNextOrderIndex = async (portalMasAccessId: string) => {
...
export class PortalMenuMapService {
  static async create(requesterId: string, reqBody: CreatePortalMenuMapRequest) {
    ...
    const orderIndex =
      request.orderIndex ?? (menu.orderIndex > 0 ? menu.orderIndex : await getNextOrderIndex(portal.masAccessId));
  ...
  static async update(requesterId: string, reqBody: UpdatePortalMenuMapRequest) {
  ...
  static async softDelete(
    requesterId: string,
    reqBody: DeletePortalMenuMapRequest
  ) {
