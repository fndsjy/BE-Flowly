...
const markOutboxSent = async (id: string, attempts: number) => {
  await prismaFlowly.caseNotificationOutbox.update({
    where: { caseNotificationId: id },
    data: {
      attempts,
...
const persistFinalMessage = async (id: string, message: string) => {
  await prismaFlowly.caseNotificationOutbox.update({
    where: { caseNotificationId: id },
    data: {
      message,
...
const buildCaseLabel = (caseType?: string | null) =>
  caseType === "PROJECT" ? "project" : "masalah";
...
const buildTemplateContext = async (item: {
  ...
  let sbuSubName =
    typeof meta.sbuSubName === "string" ? meta.sbuSubName : "";
  let sbuSubCode =
    typeof meta.sbuSubCode === "string" ? meta.sbuSubCode : "";
  let sbuSubId =
    typeof meta.sbuSubId === "number" ? meta.sbuSubId : null;

  let departmentSbuSubId: number | null = null;
  let assignedBy: string | null = null;
  let departmentCreatedBy: string | null = null;
  ...
  const originSbuSubId =
    typeof caseHeader?.originSbuSubId === "number"
  ...
  const recipient =
    item.recipientEmployeeId !== null && item.recipientEmployeeId !== undefined
  ...
  try {
    ...
    if (customTemplate?.messageTemplate) {
      ...
      const defaultTemplate = await resolveDefaultTemplate({
        ...
        caseType:
          typeof contextValue.caseType === "string" ? contextValue.caseType : null,
      });
