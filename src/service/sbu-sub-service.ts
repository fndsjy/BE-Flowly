...
const getSbuSubSnapshot = (record: Record<string, unknown>) =>
  pickSnapshot(record, SBU_SUB_AUDIT_FIELDS as unknown as string[]);
...
const resolveExplicitSbuSubAccess = async (
  requesterId: string,
  sbuSubId: number
...
export class SbuSubService {
  ...
  static async create(requesterId: string, reqBody: CreateSbuSubRequest) {
    ...
    await writeAuditLog({
      ...
      snapshot: getSbuSubSnapshot(response as unknown as Record<string, unknown>),
    });
  ...
  static async update(requesterId: string, reqBody: UpdateSbuSubRequest) {
    ...
    const changes = buildChanges(
      ...
      SBU_SUB_AUDIT_FIELDS as unknown as string[]
    );
    if (changes.length > 0) {
      await writeAuditLog({
        module: "SBU_SUB",
  ...
  static async softDelete(requesterId: string, reqBody: DeleteSbuSubRequest) {
    ...
    await writeAuditLog({
      ...
      actorId: requesterId,
      actorType: resolveActorType(requesterId),
      snapshot: getSbuSubSnapshot(snapshot as unknown as Record<string, unknown>),
    });
  ...
  /* ---------- GET BY SBU ---------- */
  static async getBySbu(requesterId: string, sbuId: number) {
    const accessContext = await getAccessContext(requesterId);
    const moduleAccessMap = await getModuleAccessMap(requesterId);
    if (!accessContext.isAdmin && !canReadModule(moduleAccessMap, "SBU_SUB")) {
      throw new ResponseError(403, "Module SBU_SUB access required");
