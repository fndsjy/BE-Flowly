...
const normalizePortalKey = (value: unknown) => {
  const text = normalizeText(value);
  return text ? text.toUpperCase() : null;
};

const portalContextFromKey = (portalKey: string): PortalContext => ({
  portalKey,
...
const getRequestValue = (req: Request, key: string) => {
  const body = req.body as Record<string, unknown> | undefined;
  return firstText(body?.[key] ?? req.query[key]);
};

const derivePortalFromReferrer = (req: Request): PortalContext | null => {
  ...
  try {
    ...
    if (pathname.includes("/supplier")) return portalContextFromKey("SUPPLIER");
    if (pathname.includes("/customer")) return portalContextFromKey("CUSTOMER");
    if (pathname.includes("/affiliate")) return portalContextFromKey("AFFILIATE");
    if (pathname.includes("/influencer")) return portalContextFromKey("INFLUENCER");
    if (pathname.includes("/community")) return portalContextFromKey("COMMUNITY");
...
const resolvePortalFromTemplate = async (
...
const resolvePortalFromStageTemplate = async (
...
const resolvePortalFromStageMaterial = async (
...
const resolvePortalFromStageExam = async (
...
const resolvePortalFromAssignment = async (
...
const resolvePortalContext = async (req: Request): Promise<PortalContext> => {
  const explicitPortalKey = normalizePortalKey(
    getRequestValue(req, "portalKey") ?? req.get("x-oms-portal")
  );
...
const normalizeRouteEntity = (path: string) => {
  const parts = path
    .split("/")
    .filter(Boolean)
    .slice(0, 2);
...
const extractEntityId = (req: Request, responseBody: unknown) => {
  const candidates: unknown[] = [];
  const body = req.body as Record<string, unknown> | undefined;
  const response = (responseBody as { response?: unknown } | undefined)?.response;
  if (response && typeof response === "object") {
    candidates.push(...Object.values(response as Record<string, unknown>));
  }
...
const buildChangesFromBefore = (
  ...
  if (method === "DELETE") {
    const deleteChanges: AuditChange[] = [];
...
const captureBeforeSnapshot = async (req: Request): Promise<BeforeSnapshot | null> => {
  ...
  if (path === "/onboarding-stage") {
    const onboardingStageTemplateId = getRequestValue(req, "onboardingStageTemplateId");
    if (!onboardingStageTemplateId) return null;
  ...
  if (path === "/onboarding-material/assignments") {
    const onboardingStageMaterialId = getRequestValue(req, "onboardingStageMaterialId");
    if (!onboardingStageMaterialId) return null;
  ...
  await writeAuditLog({
    ...
    snapshot: {
      before: beforeSnapshot?.record
        ? (sanitizeValue(beforeSnapshot.record) as Record<string, unknown>)
        : null,
...
export const auditMutationMiddleware = (
  ...
  const setupAuditCapture = (beforeSnapshot: BeforeSnapshot | null) => {
