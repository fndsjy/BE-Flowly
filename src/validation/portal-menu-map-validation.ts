...
export class PortalMenuMapValidation {
  static readonly CREATE: ZodType = portalTargetSchema
    ...
    .refine(
      (data) => Boolean(data.portalMasAccessId) || Boolean(data.portalKey),
      { message: "portalMasAccessId or portalKey is required" }
    )
    .refine(
      (data) => Boolean(data.menuMasAccessId) || Boolean(data.menuKey),
      { message: "menuMasAccessId or menuKey is required" }
    );
  ...
  static readonly UPDATE: ZodType = z
    ...
    .refine(
      (data) =>
        data.portalMasAccessId !== undefined ||
